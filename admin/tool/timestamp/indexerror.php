    ...
    foreach ($updateidarray as $key => $submissionid) {
        ...
        if (!empty($_POST[$newdate1_field])) {
            ...
            if ($newtimestamp1 > time()) {
                ...
                $current = $DB->get_record('assign_submission', [
                    'id' => $submissionid
                ], 'id, attemptnumber, assignment, userid, timemodified, timecreated', MUST_EXIST);
                ...
                /**
                 * (Optional) If you want to enforce "submission date < existing grade date"
                 * for the same attempt, you'd do that check here.
                 */

                // If all checks pass, do the DB update
                // --------------------------------------------------------------
                // NEW: Overwriting creation date (timecreated) as well.
                // --------------------------------------------------------------
                $u1 = $DB->execute("
                    UPDATE {assign_submission}
                       SET timemodified = :ts,
                           timecreated = :ts -- NEW: Overwriting creation date
                     WHERE id = :subid
        ...
        if (!empty($_POST[$newdate2_field])) {
            ...
            if ($newtimestamp2 > time()) {
                ...
                // ---------------------------------------------------------
                // *** Here is the grade_update() call to manage
