...
if (isset($_POST['updateform']) && $_POST['updateform'] != '') {
    ...
    foreach ($updateidarray as $key => $submissionid) {
        // These are the form fields for new dates
        $newdate1_field = "newdate1_".$submissionid; // submission
        $newdate2_field = "newdate2_".$submissionid; // grade
...
$setLimit  = 10; // We fix 10 records per page (avoid huge loads)
...
$list_all_assessments = $DB->get_records_sql("SELECT z.id AS assignmentid, z.name AS assignmentname, z.course FROM {assign} z");
$assessmentstr = '';
...
/** =========================================================================
 *  8) Build the Big IF/ELSE for Filters, Adding Attempt Number
 *     in every SELECT.
 *  =========================================================================*/
 ...
 * else { default }
 *
 * In each, we add "x.attemptnumber AS attemptnum"
 * and in the JOIN with {assign_grades} "g.attemptnumber = x.attemptnumber"
 */
...
if (!empty($SESSION->courseid) && empty($SESSION->studentid) && empty($SESSION->assessmentid)) {
    ...
    $sql_all = "SELECT
        ...
        x.timemodified,
        x.assignment,
        x.attemptnumber AS attemptnum,    /* <--- add attempt number */
        y.firstname,
        y.lastname,
    ...
    // Rebuild assessment list for that course
...
// If we truly want to allow "Show All" (which can be large/slow):
if ($showallstudent == 1) {
    // NOTE: This can cause performance issues with large data sets
...
foreach ($list as $row) {
    ...
    $arr[] = [
       ...
       'attemptnum'      => $attemptnum // <-- newly added for display
    ];
}
...
// ------------------------------------------------------------------------
// CLIENT-SIDE DATE VALIDATION SCRIPT (Prevents future dates)
// ------------------------------------------------------------------------
?>
<script>
document.addEventListener('DOMContentLoaded', function() {
    // Target the form that holds your date fields
    const form = document.querySelector('#timestampForm'); 
    if (form) {
        form.onsubmit = function() {
            // Grab all date inputs that match newdate1_XXX or newdate2_XXX
            const dateInputs = document.querySelectorAll('[id^="newdate1_"], [id^="newdate2_"]');
            const now = Date.now();
            ...
            for (let i = 0; i < dateInputs.length; i++) {
                ...
                const userTime = new Date(input.value).getTime();
                if (userTime > now) {
                    // If the selected date is in the future:
                    alert("You entered a future date: " + input.value + ".\nPlease correct it.");
                    input.focus();
                    return false; // Prevent form submission
                }
            }
            // If no future dates found, allow submission
            return true;
        };
    }
});
