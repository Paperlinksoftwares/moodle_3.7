    ...
    foreach ($updateidarray as $key => $submissionid) {
            ...
            if ($newtimestamp1 > time()) {
                ...
                if ($curattempt > 0) {
                    ...
                    if ($prev) {
                        ...
                        if ($newtimestamp1 <= $prev->timemodified) {
                ...
                if ($next) {
                    // Must be strictly BEFORE the next attempt
                    if ($newtimestamp1 >= $next->timemodified) {
                        echo "<script>alert('Attempt #"
        ...
        if (!empty($_POST[$newdate2_field])) {
            ...
            if ($newtimestamp2 > time()) {
                ...
                // (We do not have next/previous checks for the grade date, 
                //  but you can add them similarly if needed.)
