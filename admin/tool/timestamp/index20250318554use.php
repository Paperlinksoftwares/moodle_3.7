...
if (!empty($_POST['updateform'])) {
    ...
    foreach ($updateidarray as $key => $submissionid) {
        ...
        /** ---------------------------------------------
         * If a new submission date is provided
         * -------------------------------------------- */
        if (!empty($_POST[$newdate1_field])) {
            ...
            if ($newtimestamp1 > time()) {
                ...
                // 1) Load ALL attempts for this assignment+user, sorted by attemptnumber
                // ----------------------------------------------
                $allattempts = $DB->get_records('assign_submission', [
                    'assignment' => $assignid,
                    'userid'     => $curuserid
                ], 'attemptnumber ASC'); // sorted by attemptnumber

                // Convert to a zero-based array for easy indexing
                $allarray = array_values($allattempts);

                // 2) Find which index is the current attempt
                $pos = -1;
                foreach ($allarray as $i => $arec) {
                    if ($arec->attemptnumber == $curattempt) {
                        $pos = $i;
                        break;
                    }
                }
                ...
                // 3) Check previous attempt if it exists
                if ($pos > 0) {
                    $prevattempt = $allarray[$pos - 1];
                    if ($newtimestamp1 <= $prevattempt->timemodified) {
                        $prevnum = $prevattempt->attemptnumber + 1; 
                        $thisnum = $curattempt + 1;
                        echo "<script>alert('Attempt #{$thisnum} date must be AFTER Attempt #{$prevnum}!');</script>";
                        // skip this update
                        continue;
                    }
                }

                // 4) Check next attempt if it exists
                if ($pos < count($allarray) - 1) {
                    $nextattempt = $allarray[$pos + 1];
                    if ($newtimestamp1 >= $nextattempt->timemodified) {
                        $thisnum = $curattempt + 1;
                        $nextnum = $nextattempt->attemptnumber + 1;
                        echo "<script>alert('Attempt #{$thisnum} date must be BEFORE Attempt #{$nextnum}!');</script>";
                        // skip
                        continue;
                    }
                }

                // If all checks pass, do the update
                $u1 = $DB->execute("
        ...
        /** ---------------------------------------------
         * If a new grade date is provided
         * -------------------------------------------- */
