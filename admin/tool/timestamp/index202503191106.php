    ...
    foreach ($updateidarray as $key => $submissionid) {
        ...
        if (!empty($_POST[$newdate2_field])) {
            ...
            if ($newtimestamp2 > time()) {
                ...
                // Get the current record in {assign_grades} so we know the attemptnumber
                $thisgrade = $DB->get_record('assign_grades', [
                    'id' => $gid
                ], 'id, attemptnumber, userid, assignment, timemodified', MUST_EXIST);

                $thisattemptnum = $thisgrade->attemptnumber;

                // Load ALL grade attempts for this assignment+user, sorted by attemptnumber
                ...
                // Check previous grade attempt date
                if ($gradepos > 0) {
