...
if(isset($_POST['updateform']) && $_POST['updateform']!='' )
{
    ...
    foreach($_POST['updateidarray'] as $key=>$val)
    {
        ...
        if(@$_POST[$newdate1]!='')
        {
           ...
           $ufile_1 = $DB->execute("UPDATE {files} set `timemodified` = '".strtotime($_POST[$newdate1])."' WHERE `component` = 'assignsubmission_file' AND `filearea` = 'submission_files' AND `itemid` = '".$val."' AND `source`!='' AND `filename`!=''");
        }
        if(@$_POST[$newdate2]!='')
        {
			//echo "UPDATE {assign_grades} set `timemodified` = '".strtotime($_POST[$newdate2])."' WHERE `userid` = '".$useridarray[$c]."' AND `assignment` = '".$assidarray[$c]."' AND `id` = '".$updateidarray[$c]."'";
            $u2 = $DB->execute("UPDATE {assign_grades} set `timemodified` = '".strtotime($_POST[$newdate2])."' WHERE `userid` = '".$useridarray[$c]."' AND `assignment` = '".$assidarray[$c]."' AND `id` = '".$gradeitemid[$c]."'");
           // $ufile_2 = $DB->execute("UPDATE {files} set `timemodified` = '".strtotime($_POST[$newdate2])."' WHERE `component` = 'assignfeedback_file' AND `filearea` = 'feedback_files' AND `itemid` = '".$gradeitemid[$c]."' AND `userid` = '".$graderidarray[$c]."'");          
			$ufile_2 = $DB->execute("UPDATE {files} set `timemodified` = '".strtotime($_POST[$newdate2])."' WHERE `component` = 'assignfeedback_file' AND `filearea` = 'feedback_files' AND `itemid` = '".$gradeitemid[$c]."'");          
			
			//echo "UPDATE {files} set `timemodified` = '".strtotime($_POST[$newdate2])."' WHERE `component` = 'assignfeedback_file' AND `filearea` = 'feedback_files' AND `itemid` = '".$gradeitemid[$c]."' AND `userid` = '".$graderidarray[$c]."'"; die;
			$list_select = $DB->get_record_sql("SELECT gi.id as gid FROM {grade_items} as gi WHERE gi.iteminstance = '".$assidarray[$c]."'");
		
			$grade_3 = $DB->execute("UPDATE {grade_grades} set `timemodified` = '".strtotime($_POST[$newdate2])."' WHERE `itemid` = '".$list_select->gid."' AND `userid` = '".$useridarray[$c]."'");               
...
{
    $sql_all = 'SELECT x.id as rowid , x.userid, x.timemodified, x.assignment, y.firstname , y.lastname, y.id , us.firstname as graderfirstname , us.lastname as graderlastname , us.id as graderuserid, z.id as assignmentid , z.name as assignmentname , z.course , g.userid as uid , g.id as gradeitemid, g.grader as graderid , g.assignment as ass , g.timemodified as gtimemodified FROM {assign_submission} as x JOIN {user} as y ON x.userid = y.id '
        . ' JOIN {assign} as z ON x.assignment = z.id LEFT JOIN {assign_grades} as g ON g.assignment = x.assignment AND g.userid = x.userid AND g.attemptnumber = x.attemptnumber LEFT JOIN {user} as us ON g.grader = us.id WHERE x.userid ='.$SESSION->studentid.' AND z.course = '.$SESSION->courseid.' AND z.id = '.$SESSION->assessmentid.' '.$SESSION->sortorder;
 
     $sql = 'SELECT x.id as rowid , x.userid, x.timemodified, x.assignment, y.firstname , y.lastname, y.id , us.firstname as graderfirstname , us.lastname as graderlastname , us.id as graderuserid, z.id as assignmentid , z.name as assignmentname , z.course , g.userid as uid , g.id as gradeitemid, g.grader as graderid , g.assignment as ass , g.timemodified as gtimemodified FROM {assign_submission} as x JOIN {user} as y ON x.userid = y.id '
        . ' JOIN {assign} as z ON x.assignment = z.id LEFT JOIN {assign_grades} as g ON g.assignment = x.assignment AND g.userid = x.userid  AND g.attemptnumber = x.attemptnumber LEFT JOIN {user} as us ON g.grader = us.id WHERE x.userid ='.$SESSION->studentid.' AND z.course = '.$SESSION->courseid.' AND z.id = '.$SESSION->assessmentid.' '.$SESSION->sortorder.' LIMIT '.$pageLimit.' , '.$setLimit;
 $list_all_assessments = $DB->get_records_sql("SELECT z.id as assignmentid , z.name as assignmentname , z.course FROM 
         mdl_assign as z WHERE z.course =".$SESSION->courseid." AND z.grade!='0'");
$assessmentstr = '';
foreach($list_all_assessments as $list_all_assessments)
...
else if(@$SESSION->courseid>0 && @$SESSION->studentuserid>0 &&  @$SESSION->assessmentid>0)
{
    $sql_all = 'SELECT x.id as rowid , x.userid, x.timemodified, x.assignment, y.firstname , y.lastname, y.id , us.firstname as graderfirstname , us.lastname as graderlastname , us.id as graderuserid, z.id as assignmentid , z.name as assignmentname , z.course , g.userid as uid , g.id as gradeitemid, g.grader as graderid , g.assignment as ass , max(g.timemodified) as gtimemodified FROM {assign_submission} as x JOIN {user} as y ON x.userid = y.id '
        . ' JOIN {assign} as z ON x.assignment = z.id LEFT JOIN {assign_grades} as g ON x.assignment = g.assignment AND g.userid = x.userid AND g.id = (SELECT max(`id`) as maxid FROM {assign_grades} agr WHERE agr.userid = x.userid AND agr.assignment = x.assignment) LEFT JOIN {user} as us ON g.grader = us.id WHERE x.userid ='.$SESSION->studentuserid.' AND z.course = '.$SESSION->courseid.' AND z.id = '.$SESSION->assessmentid.' '.$SESSION->sortorder;
 
     $sql = 'SELECT x.id as rowid , x.userid, x.timemodified, x.assignment, y.firstname , y.lastname, y.id , us.firstname as graderfirstname , us.lastname as graderlastname , us.id as graderuserid, z.id as assignmentid , z.name as assignmentname , z.course , g.userid as uid , g.id as gradeitemid, g.grader as graderid , g.assignment as ass , g.timemodified as gtimemodified FROM {assign_submission} as x JOIN {user} as y ON x.userid = y.id '
        . ' JOIN {assign} as z ON x.assignment = z.id LEFT JOIN {assign_grades} as g ON g.assignment = x.assignment AND g.userid = x.userid AND g.id = (SELECT max(`id`) as maxid FROM {assign_grades} agr WHERE agr.userid = x.userid AND agr.assignment = x.assignment) LEFT JOIN {user} as us ON g.grader = us.id WHERE x.userid ='.$SESSION->studentuserid.' AND z.course = '.$SESSION->courseid.' AND z.id = '.$SESSION->assessmentid.' '.$SESSION->sortorder.' LIMIT '.$pageLimit.' , '.$setLimit;
 $list_all_assessments = $DB->get_records_sql("SELECT z.id as assignmentid , z.name as assignmentname , z.course FROM 
         mdl_assign as z WHERE z.course =".$SESSION->courseid." AND z.grade!='0'");
$assessmentstr = '';
foreach($list_all_assessments as $list_all_assessments)
