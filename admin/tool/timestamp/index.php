...
// If you have a page in your admin settings, you might do:
...
// Buttons for Exporting Data to the Excel Start

// ----------------------------------------------------------------------------
// Accordion container — clicking the title toggles visibility of its contents
// ----------------------------------------------------------------------------
echo '<div class="panel-group" id="exportAccordion">';

// ----------------------------------------------------------------------------
// 1) Panel for “Export Options”
// ----------------------------------------------------------------------------
echo '<div class="panel panel-default">';
// Panel heading with toggle link
echo '<div class="panel-heading">';
echo '<h4 class="panel-title">';
// data-toggle & href must match the collapse ID
echo '<a data-toggle="collapse" href="#exportCollapse" '
   . 'aria-expanded="false" aria-controls="exportCollapse">';
    // get the nicely translated text, then add a caret-down icon
    echo get_string('exportoptions','tool_timestamp')
       . ' <i class="fa fa-caret-down" aria-hidden="true"></i>';
echo '</a>';
echo '</h4>';
echo '</div>';

// Collapsible body (initially hidden)
echo '<div id="exportCollapse" class="panel-collapse collapse">';
echo '<div class="panel-body">';

// ----------------------------------------------------------------
// Filtered‐export button
//   – Carries over “showall” param if you previously clicked “Show all”
// ----------------------------------------------------------------
// prepare URL params
$params = [];
if (!empty($SESSION->showallstudent)) {
    $params['showall'] = 1;
}
// build the download URL
$downloadurl = new moodle_url('/admin/tool/timestamp/download.php', $params);
// render the button
...
// ----------------------------------------------------------------
// Export‐all button
//   – Ignores filters and dumps every record
// ----------------------------------------------------------------
$exportallurl = new moodle_url('/admin/tool/timestamp/download_all.php');
echo ' '; // small space between buttons
echo html_writer::link(
    $exportallurl,
...
// Buttons for Exporting Data to the Excel End
 ...
 *     Overwrite submission timemodified & timecreated, plus grade timemodified
 *     Then call grade_update() to keep logs/history consistent
 *  =========================================================================*/
if (!empty($_POST['updateform'])) {
    ...
    foreach ($updateidarray as $key => $submissionid) {
        ...
        /** ----------------------------------------------------------------
         * A) If a new submission date is provided (overwrites timecreated too)
         * ----------------------------------------------------------------*/
        if (!empty($_POST[$newdate1_field])) {
            ...
            if ($newtimestamp1 > time()) {
                ...
                // Overwrite both timemodified and timecreated
                $u1 = $DB->execute("
                    UPDATE {assign_submission}
                       SET timemodified = :ts1,
                           timecreated  = :ts2
                     WHERE id = :subid
                ", ['ts1' => $newtimestamp1, 'ts2' => $newtimestamp1, 'subid' => $submissionid]);
        ...
        if (!empty($_POST[$newdate2_field])) {
            ...
            if ($newtimestamp2 > time()) {
                ...
                if (!empty($thatsub)) {
                    ...
                    $minrequired    = $submissiontime + 1800; // 30 min = 1800 sec
                    if ($newtimestamp2 < $minrequired) {
                        $gradeattdisplay = $thisattemptnum + 1;
                        echo "<script>alert('Attempt #{$gradeattdisplay} grade date must be >= 30 mins after submission!');</script>";
                        continue; // skip
                    }
                }
                ...
                if ($gradepos > 0) {
                    ...
                    if ($newtimestamp2 <= $prevgrade->timemodified) {
                        ...
                        echo "<script>alert('Attempt #{$thisnum} grade date must be AFTER Attempt #{$prevnum}!');</script>";
                        continue; // skip
                    }
                }
                ...
                if ($gradepos < count($allgrarr) - 1) {
                    ...
                    if ($newtimestamp2 >= $nextgrade->timemodified) {
                        ...
                        echo "<script>alert('Attempt #{$thisnum} grade date must be BEFORE Attempt #{$nextnum}!');</script>";
                        continue; // skip
                    }
                }

                // 6) Update the DB manually
                $u2 = $DB->execute("
                ...
                // ---------------------------------------------------------
                // *** grade_update() to keep grade logs/history consistent ***
                // ---------------------------------------------------------
                if ($u2) {
                    ...
                    // Get the existing final grade record from {grade_grades}
                    $existingfinal = $DB->get_record('grade_grades', [
                        'itemid' => $gradeitem->id,
                        'userid' => $userid
                    ], '*', IGNORE_MULTIPLE);

                    // If no grade, we can set to -1 ("No Grade")
                    $finalgrade = -1;
                    if (!empty($existingfinal) && $existingfinal->rawgrade !== null) {
                        $finalgrade = (float)$existingfinal->rawgrade;
                    }

                    // Prepare the array for grade_update()
                    $grades = [];
                    $grades[$userid] = (object)[
                        'userid'     => $userid,
                        'rawgrade'   => $finalgrade,
                        'dategraded' => $newtimestamp2, // Updated grade date
                    ];
...
?>
    ...
    <script>
        window.location.href = '<?php echo $urltogo; ?>';
    </script>
<?php
    exit;
}
...
/** =========================================================================
 *  8) Build the Big IF/ELSE for Filters (9 scenarios)
 *  =========================================================================*/
if (!isset($SESSION->courseid)) {
    $SESSION->courseid = '';
}
if (!isset($SESSION->studentid)) {
    $SESSION->studentid = '';
}
if (!isset($SESSION->studentuserid)) {
    $SESSION->studentuserid = '';
}
if (!isset($SESSION->assessmentid)) {
    $SESSION->assessmentid = '';
}
...
if (!empty($SESSION->courseid) && empty($SESSION->studentid) && empty($SESSION->assessmentid) && empty($SESSION->studentuserid)) {
    ...
    // 9) No filters => show nothing or skip
} else {
    $sql_all = '';
    $sql = '';
}
...
foreach ($list as $row) {
    ...
    if (!empty($row->gtimemodified)) {
        ...
        $gtimemodified = 'NA';
        $grade_exists  = 'disabled'; // e.g., disables date editing if no grade
    }
