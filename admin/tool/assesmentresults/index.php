...
$PAGE->set_heading('Assessment Results & Dates');
require_login();
...
$selectedyear = optional_param('year', $currentyear, PARAM_INT);
if (!in_array($selectedyear, $years, true)) {
    print_error('invalidyear', 'tool_assesmentresults');
}

$selectedterm = optional_param('term', 0, PARAM_INT);
if ($selectedterm >= 1 && $selectedterm <= 4) {
    $SESSION->selectterm = $selectedterm;
    if ($selectedterm == 1) {
        $SESSION->termclause = '(MONTH(FROM_UNIXTIME(course.startdate)) = "1" OR MONTH(FROM_UNIXTIME(course.startdate)) = "2" OR MONTH(FROM_UNIXTIME(course.startdate)) = "3")';
    } else if ($selectedterm == 2) {
        $SESSION->termclause = '(MONTH(FROM_UNIXTIME(course.startdate)) = "4" OR MONTH(FROM_UNIXTIME(course.startdate)) = "5" OR MONTH(FROM_UNIXTIME(course.startdate)) = "6")';
    } else if ($selectedterm == 3) {
        $SESSION->termclause = '(MONTH(FROM_UNIXTIME(course.startdate)) = "7" OR MONTH(FROM_UNIXTIME(course.startdate)) = "8" OR MONTH(FROM_UNIXTIME(course.startdate)) = "9")';
    } else {
        $SESSION->termclause = '(MONTH(FROM_UNIXTIME(course.startdate)) = "10" OR MONTH(FROM_UNIXTIME(course.startdate)) = "11" OR MONTH(FROM_UNIXTIME(course.startdate)) = "12")';
    }
} else {
...
$SESSION->selectyear = $selectedyear;
$SESSION->yearclause = ' YEAR(FROM_UNIXTIME(course.startdate)) = "' . $selectedyear . '"';

$yearoptions = array_reverse($years);
$yearoptions = array_map(function($year) use ($selectedyear) {
...
echo $OUTPUT->header();
echo $OUTPUT->heading('Assessment Results & Dates');
global $SESSION;
...
else if($SESSION->termclause!='' && $SESSION->yearclause!='')
{
	...
	if($SESSION->termclause!='')
	{
		$termclause = $SESSION->termclause;
	}
	if($SESSION->yearclause!='')
	{
		$yearclause = $SESSION->yearclause;
	}
   $sql_all = 'SELECT x.id as rowid , x.userid, x.timemodified, x.assignment, y.firstname , y.lastname, y.id , '
             . ' x.assignment as assignmentid , z.name as assignmentname , g.grade as recorded_grade , '
             . 'z.course , g.id as gradeitemid , g.userid as uid , g.grader as graderid , g.assignment as ass , g.timemodified as gtimemodified , '
...
{
    $sqlcount = preg_replace('/^SELECT\s+.+?\s+FROM/si', 'SELECT x.assignment, x.userid FROM', $sql_all);
    $sqlcount = preg_replace('/ORDER BY.+$/si', '', $sqlcount);
...
$renderable = new \tool_assesmentresults\output\index_page(
     $arr,
     $coursestr,
     $assessmentstr,
     $studentsstr,
...
{
    echo '<br/><br/>';
    echo displayPaginationHere($totalrecords,$setLimit,$page); // Call the Pagination Function to display Pagination.
}
echo $OUTPUT->footer();