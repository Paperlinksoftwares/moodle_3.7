...
if(isset($_REQUEST['termsel']))
{
	$term = $_REQUEST['termsel'];
}
else
{
	$term = 1;
...
if(isset($_POST['updateform']) && $_POST['updateform']!='' )
{
    ...
    $urltogo= $CFG->wwwroot.'/admin/tool/submissionsbasic/indexwork.php?update=1&sort='.$sort.'&sorttype='.$_POST['sorttype'].'&showallstudent='.$_REQUEST['showallstudent'].'&studentid='.$_POST['graderid'].'&yearsel='.$_POST['yearselect'].'&termsel='.$_POST['termval'];
    ?>
...
if(isset($grader_id) && $grader_id>0)
{
	...
	JOIN {user} as y ON x.userid = y.id '
		...
		LEFT JOIN {user} as us ON g.grader = us.id WHERE x.status ="submitted" and g.grader ='.$grader_id.' AND YEAR(FROM_UNIXTIME(course.startdate)) = '.$year.' AND '.$term_query.' '.$set_query.' '.$SESSION->sortorder;
