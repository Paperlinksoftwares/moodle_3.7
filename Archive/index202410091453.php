...
foreach($roleassignments as $key=>$val)
{
	if($val->roleid!=5)
	{
		$if_student = 0;
...
$sql_8="SELECT c.id, c.shortname, c.summary, c.idnumber  
FROM mdl_course c 
JOIN mdl_enrol en ON en.courseid = c.id 
JOIN mdl_user_enrolments ue ON ue.enrolid = en.id 
WHERE ue.userid = '".$USER->id."' AND c.id != '379'";

$record_8 = $DB->get_records_sql($sql_8);

//print count($record_8);
...
{
    $u2 = $DB->execute("INSERT INTO `mdl_roleplay_confirmation`(`id`,`user_id`,`term` , `confirm`,`date`) VALUES(NULL, '".@$USER->id."' , '".@date('Y')."|".$current_term."' , '".$_GET['confirm']."', '".@date("Y-m-d h:i:s")."')"); 
...
{
$sql = "SELECT uen.enrolid , en.courseid from {user_enrolments} as uen LEFT JOIN {enrol} as en ON en.id = uen.enrolid LEFT JOIN {course} as course ON course.id = en.courseid WHERE uen.userid = '".@$USER->id."' AND ( MONTH(FROM_UNIXTIME(course.startdate)) = '4' OR MONTH(FROM_UNIXTIME(course.startdate)) = '5' OR MONTH(FROM_UNIXTIME(course.startdate)) = '6') AND YEAR(FROM_UNIXTIME(course.startdate)) = '".@date('Y')."'";
...
$sql_form_check3 = "SELECT count(`id`) as countrow FROM `zoho_forms` WHERE `form_name` = 'EnrollmentForm' AND `userid` = '".$USER->id."' AND `term` = '".@date('Y')."|".$current_term."'";
...
$sql_form_check4 = "SELECT count(`id`) as countrow FROM `zoho_forms` WHERE `form_name` = 'FeedbackForm' AND `userid` = '".$USER->id."' AND `term` = '".@date('Y')."|".$current_term."'";
...
$sql_form_check5 = "SELECT count(`id`) as countrow FROM `zoho_forms` WHERE `form_name` = 'TransitionForm' AND `userid` = '".$USER->id."' AND `term` = '".@date('Y')."|".$current_term."'";

$form_check5 = $DB->get_record_sql($sql_form_check5);

//STUDENTS ROLEPLAY FORM

$sql_form_check6 = "SELECT count(`id`) as countrow FROM `mdl_roleplay_confirmation` WHERE `user_id` = '".$USER->id."' AND `term` = '".@date('Y')."|".$current_term."'";
...
//STUDENTS ONLINE ACTIVITY MODULE POP UP NOTIFICATION

// ONLINE ACTIVITY - ASSIGNMENT WITHOUT GRADING

$activity_arr_assignment = array();
$show_assign_popup = 0;
$sql_user_course = "SELECT uen.enrolid , en.courseid from {user_enrolments} as uen LEFT JOIN {enrol} as en ON en.id = uen.enrolid LEFT JOIN {course} as course 
ON course.id = en.courseid WHERE uen.userid = '".@$USER->id."' AND ( MONTH(FROM_UNIXTIME(course.startdate)) = '10' OR MONTH(FROM_UNIXTIME(course.startdate)) = '11' 
OR MONTH(FROM_UNIXTIME(course.startdate)) = '12') AND YEAR(FROM_UNIXTIME(course.startdate)) = '".@date('Y')."'";
$list_user_course = $DB->get_records_sql($sql_user_course);

foreach($list_user_course as $b=>$p)
{
	...
	AND b.`status` IS NOT NULL AND b.`status` = 'submitted' AND b.`userid` = '".$USER->id."'
	LEFT JOIN `mdl_course_modules` as c ON c.`course` = '".$p->courseid."' AND c.`instance` = a.`id` AND c.`module` = '29'
	
 WHERE a.`course` = ".$p->courseid." AND a.`grade` = 0 AND a.duedate != 0 AND FROM_UNIXTIME(a.duedate,'%Y-%m-%d %h %i %s') > NOW() 
...
if(count($form_check_assign_popup)>0)
{
	foreach($form_check_assign_popup as $x=>$y)
	{
		if($y->userid !=$USER->id && trim($y->status) != 'submitted')
		{
			
			$show_assign_popup = 1;
			$activity_arr_assignment[$p->courseid][$y->id]['id']=$y->id;
			$activity_arr_assignment[$p->courseid][$y->id]['moduleid']=$y->moduleid;	
			$activity_arr_assignment[$p->courseid][$y->id]['name']=$y->name;	
		}
	}
}
...
//echo '<pre>';
//print_r($activity_arr_assignment);
//die;


// ONLINE ACTIVITY - FORUM DISCUSSION
...
foreach($list_user_course as $b=>$p)
{
	$sql_form_check_forum_popup = "SELECT a.id , a.name , a.duedate , a.cutoffdate , b.firstpost, b.userid , c.id as moduleid FROM `mdl_forum` as a 
	LEFT JOIN `mdl_forum_discussions` as b
	ON a.`id` = b.`forum` 
	LEFT JOIN `mdl_course_modules` as c ON c.`course` = '".$p->courseid."' AND c.`instance` = a.`id` AND c.`module` = '5'
	WHERE a.`course` = ".$p->courseid." AND a.`type` = 'general' AND a.duedate != 0 
	AND FROM_UNIXTIME(a.duedate,'%Y-%m-%d %h %i %s') > NOW() 
    AND FROM_UNIXTIME(a.duedate,'%Y-%m-%d %h %i %s')< (NOW() + INTERVAL 3 DAY) AND b.`firstpost` IS NULL AND ( b.`userid` = '".$USER->id."' OR b.`userid` IS NULL)";
	
	//echo '<br/>';

$form_check_forum_popup = $DB->get_records_sql($sql_form_check_forum_popup);
if(count($form_check_forum_popup)>0)
{
	$activity_arr_forum[$p->courseid][]=$form_check_forum_popup;
	$show_forum_popup = 1;
}
unset($form_check_forum_popup);
}

//echo '<pre>';
//print_r($activity_arr_forum);


// ONLINE ACTIVITY - QUIZZ
...
foreach($list_user_course as $b=>$p)
{
	$sql_form_check_quizz_popup = "SELECT a.id , a.name , a.timeclose , b.attempt, b.userid , c.id as moduleid FROM `mdl_quiz` as a LEFT JOIN `mdl_quiz_attempts` as b
	ON a.`id` = b.`quiz`
	LEFT JOIN `mdl_course_modules` as c ON c.`course` = '".$p->courseid."' AND c.`instance` = a.`id` AND c.`module` = '12'
	WHERE a.`course` = ".$p->courseid." AND a.timeclose != 0 AND FROM_UNIXTIME(a.timeclose,'%Y-%m-%d %h %i %s') > NOW() 
 AND FROM_UNIXTIME(a.timeclose,'%Y-%m-%d %h %i %s')< (NOW() + INTERVAL 3 DAY) AND b.`attempt` IS NULL AND ( b.`userid` = '".$USER->id."' OR b.`userid` IS NULL)";
 

$form_check_quizz_popup = $DB->get_records_sql($sql_form_check_quizz_popup);
if(count($form_check_quizz_popup)>0)
{
	$activity_arr_quizz[$p->courseid][]=$form_check_quizz_popup;
	$show_quiz_popup = 1;
}
unset($form_check_quizz_popup);
}

//echo '<pre>';
//print_r($activity_arr_quizz);
//die;
...
if($form_check3->countrow==0)
{
	...
	$sql_fetch_username = $DB->get_records_sql("SELECT `user_id` , `user_name` from `mdl_forms_students_access` WHERE `term` = '".$current_term."' AND `year` = '".@date('Y')."' AND `form_type` = '1'");
...
if(in_array($USER->id,$all_students_id)==true && in_array($USER->id,$exclude_students_3)==false && count($record_8)>0)
{
...
/// ONLINE ACTIVITY POP MODAL SHOW/HIDE
if($form_check3->countrow>0 && ( $show_quiz_popup == 1 || $show_assign_popup == 1 || $show_forum_popup == 1 ))
{
	$modal_open=true;
	$open9 = true;
}
...
if($form_check4->countrow==0 && $form_check3->countrow>0)
{
	...
	$sql_fetch_username = $DB->get_records_sql("SELECT `user_id` , `user_name` from `mdl_forms_students_access` WHERE `term` = '".$current_term."' AND `year` = '".@date('Y')."' AND `form_type` = '2'");
	foreach($sql_fetch_username as $key=>$val)
	{
...
if($modal_open==true && ($open3==true || $open4==true || $open5==true || $open6==true) && $open_new==false)
{
...
if((@date('m')=='06' || @date('m')=='09') && !isset($_GET['clicked']))
{
	
$sql_form_check_view = "SELECT count(`id`) as countrow FROM `mdl_user_popuprejoin` WHERE `user_id` = '".$USER->id."' AND `term` = '".$current_term."' AND `year` = '".@date('Y')."'";

$form_check_view = $DB->get_record_sql($sql_form_check_view);
if($form_check_view->countrow==0)
{
?>
<script>
...
$(document).ready(function(){
	...
	$.ajax({
                url: "http://localhost/accit-moodle/accit/onlineactivity_notification_cancels.php?user_id=<?php echo $USER->id; ?>&year=<?php echo @date('Y'); ?>&term=<?php echo $current_term; ?>",
                type: "GET",
                cache: false,
                success: function(d) {
                   // alert(d);
...
<script>
   ...
   $('#popup_onlineactivity_notification').modal('show');
...
$(document).ready(function(){
    ...
    $("#myModalCustom").click(function(){
    
	
	$.ajax({
                url: "http://localhost/accit-moodle/accit/onlinepopup_notification.php?user_id=<?php echo $USER->id; ?>&year=<?php echo @date('Y'); ?>&term=<?php echo $current_term; ?>",
                type: "GET",
                cache: false,
                success: function(d) {
                  //  alert(d);
...
<script>
   ...
   $('#popup_feedbackform_notification').modal('show');
...
$(document).ready(function(){
    ...
    $("#closenowfeedback").click(function(){
    
	
	$.ajax({
                url: "http://localhost/accit-moodle/accit/feedbackform_notification_cancels.php?user_id=<?php echo $USER->id; ?>&year=<?php echo @date('Y'); ?>&term=<?php echo $current_term; ?>",
                type: "GET",
                cache: false,
                success: function(d) {
                   // alert(d);
...
<?php } //if(!isset($_GET['clicked'])) {

if(isset($_GET['clickedoff'])) {	?>
  <div class="modal fade" id="myModalCustom" role="dialog" style="display: none;  top:80px;">
    ...
    <div class="modal-dialog">
      ...
      <!--  <div class="modal-header" style="padding: 24px!important; background-color: #ccc!important; padding: 0!important;">
        ...
        <div class="modal-body">
            <div  >
			<br/>
<h4>Exciting News: We're Transitioning Back to In-Person Learning</h4>
<br/>
Hello Students,
<br/><br/>
Great news! We're transitioning from remote learning back to in-person classes from July 10, 2023. Please note:
<br/><br/>
<strong>Start Date:</strong> In-person classes start on July 10.
<br/><br/>
<strong>Class Schedules:</strong> Expect some schedule changes. We'll communicate these to you in advance.
<br/><br/>
<strong>Support:</strong> We're here for you. Any questions or concerns? Please reach out at <a href="mailto: admin@accit.nsw.edu.au">admin@accit.nsw.edu.au</a>
<br/><br/>
We're excited to reconnect in the classroom and are committed to ensuring a safe, enriching environment. Stay tuned for more details soon!
           <br/>   	<!--<button style="background-color: #008CBA!important; color: white!important;" class="button button2" 
			onclick="javascript: window.location.href='<?php echo $CFG->wwwroot; ?>/index.php?clicked=1';">Great! Continue to Moodle</button> -->
			
			  <div class="modal-footer">
                    <button style="background-color: #008CBA!important; color: white!important;"  id="closenowpopup" type="button" class="btn btn-secondary" data-dismiss="modal">Okay, Continue to Moodle</button>
...
if($open9==true)
{
   ...
   <div class="modal fade" id="popup_onlineactivity_notification" role="dialog" style="display: none;  top:80px;">
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content" style="overflow-y: scroll!important; height:auto!important;">
        ...
        <div class="modal-header" style="padding: 24px!important; padding-left: 16px; background-color: #ccc!important; padding: 0!important;">
            ...
            <h4 class="modal-title" ><strong>Online Activity Notifications</strong></h4>
        </div> 
        ...
        <div class="modal-body">
			...
			<div></div>
             <div>Dear <?php echo $USER->firstname." ".$USER->lastname; ?><br/>
			  Please access weekly online activity before the due date.
</div><br/><br/>
			  <?php 
...
//echo '<pre>';
//print_r($activity_arr_assignment);
if($show_assign_popup==1)
{
			  foreach($activity_arr_assignment as $p=>$t)
			  {
				  ...
				  echo '<a target="_blank" href="'.$CFG->wwwroot.'/mod/assign/view.php?id='.$x['moduleid'].'">'.$x['name'].'</a>';
				  echo '<hr>'; 
					  }
...
//echo '<pre>';
//print_r($activity_arr_assignment);
if($show_forum_popup == 1)
{
			  foreach($activity_arr_forum as $p=>$t)
			  {
				  foreach($t as $k=>$v)
				  {
					  foreach($v as $w=>$x)
				  ...
				  echo '<a target="_blank" href="'.$CFG->wwwroot.'/mod/forum/view.php?id='.$x->moduleid.'">'.$x->name.'</a>';
				  echo '<hr>'; 
					  }
				  }
			  } 
...
//echo '<pre>';
//print_r($activity_arr_quizz);
if($show_quiz_popup==1)
{
			  foreach($activity_arr_quizz as $p=>$t)
			  {
				  foreach($t as $k=>$v)
				  {
					  foreach($v as $w=>$x)
				  ...
				  echo '<a target="_blank" href="'.$CFG->wwwroot.'/mod/quiz/view.php?id='.$x->moduleid.'">'.$x->name.'</a>';
				  echo '<hr>'; 
					  }
				  }
			  } 
...
<?php } ?>
   ...
   <!-- FEEDBACK FORM UPDATED VERSION -->
  
  
   
<?php // if(@$USER->id!='' && $open4==true && $open5!=true) 
	
if(@$USER->id!='' && $open4==true) 
	
//if(@$USER->id!='' && $USER->id == 867) 
{
	?>
   <div class="modal fade" id="popup_feedbackform_notification" role="dialog" style="display: none;  top:80px;">
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content" style="overflow-y: scroll!important; height:auto!important;">
        ...
        <div class="modal-header" style="padding: 24px!important; padding-left: 16px; background-color: #ccc!important; padding: 0!important;">
            ...
            <h4 class="modal-title" ><strong>Feedback Form Submission</strong></h4>
        </div> 
        ...
        <div class="modal-body">
			...
			<div></div>
             <div>Dear <?php echo $USER->firstname." ".$USER->lastname; ?><br/>
			 Please take two minutes to complete this Feedback form.
Your responses are extremely valuable to us.
<br/><br/>
Click on link below!
</div><br/>
		    ...
		    <div class="modal-footer">
                    <button id="closenowfeedback" type="button" class="btn btn-secondary" data-dismiss="modal">Skip for Now</button>
...
if($if_student==1)
{
	include('grade/report/overview/studentgradeprogress.php');
	echo '<br/><br/>';
}
//PROGRESSEND
