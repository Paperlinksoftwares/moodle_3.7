...
/*
$payment_due_list1 = array("a001413", "1462", "a001048", "1379", "1499", "1479", "1388", "1357", 
"1389");

$payment_due_list2 = array("1485", "1334", "a001342", "1489", "1465", "1331", "1548", "1467", "1519", "1552", 
"1502", "1523", "1472", "1551");

$exclude_students_payment1 = array();
$exclude_students_payment2 = array();

foreach($payment_due_list1 as $mc1=>$nc1)
{
	$sql_exclude_payment1 = "SELECT `id`  FROM `mdl_user` WHERE `username` = '".$nc1."'";
	$row_exclude_payment1 = $DB->get_record_sql($sql_exclude_payment1);
	$exclude_students_payment1[] = $row_exclude_payment1->id;	
	unset($sql_exclude_payment1);
	unset($row_exclude_payment1);
}

foreach($payment_due_list2 as $mc2=>$nc2)
{
	$sql_exclude_payment2 = "SELECT `id`  FROM `mdl_user` WHERE `username` = '".$nc2."'";
	$row_exclude_payment2 = $DB->get_record_sql($sql_exclude_payment2);
	$exclude_students_payment2[] = $row_exclude_payment2->id;	
	unset($sql_exclude_payment2);
	unset($row_exclude_payment2);
}

if(in_array($USER->id,$exclude_students_payment1)==true && in_array($USER->id,$exclude_students_payment2)==false)
{
	header('Location: http://localhost/accit-moodle/accit/accountrestriction/index_1.html');
	exit();
}

else if(in_array($USER->id,$exclude_students_payment1)==false && in_array($USER->id,$exclude_students_payment2)==true)
{
	header('Location: http://localhost/accit-moodle/accit/accountrestriction/index_2.html');
	exit();
}
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content" style="overflow-y: scroll!important; height:auto!important;">
        ...
        <div class="modal-body">
			...
			<div></div>
             <div>Dear <?php echo $USER->firstname." ".$USER->lastname; ?><br/>
			 Team ACCIT would appreciate if you take two minutes to give us your termly feedback!

Your feedback is important for our continuous improvement.
<br/><br/>
Click on link below!
</div><br/>
