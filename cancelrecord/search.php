...
<?php
$sql_begin = "SELECT d.id , d.date , u.firstname, u.lastname , u.email, u.id as userid, c.fullname, c.shortname, c.id as courseid , a.name as attendance_name FROM ".$SETTINGS["data_table"]." 
as d LEFT JOIN `mdl_user` as u ON d.user_id = u.id LEFT JOIN `mdl_course` as c ON c.id = d.course_id LEFT JOIN `mdl_attendance` as a 
ON a.course = d.course_id ";
...
if ($_REQUEST["from"]<>'' and $_REQUEST["to"]<>'') {
	$sql = $sql_begin." WHERE d.date >= '".@date("Y-m-d", strtotime($_REQUEST["from"]))."' AND d.date <= '".@date("Y-m-d", strtotime($_REQUEST["to"]))."'".$search_string;
	$_SESSION['from']=$_REQUEST["from"];
	$_SESSION['to']=$_REQUEST["to"];
} else if ($_REQUEST["from"]<>'') {
	$sql = $sql_begin." WHERE d.date >= '".@date("Y-m-d", strtotime($_REQUEST["from"]))."'".$search_string;
	$_SESSION['from']=$_REQUEST["from"];
} else if ($_REQUEST["to"]<>'') {
	$sql = $sql_begin." WHERE d.date <= '".@date("Y-m-d", strtotime($_REQUEST["to"]))."'".$search_string;
	$_SESSION['to']=$_REQUEST["to"];
} else {
...
<table width="100%" border="1" cellspacing="0" cellpadding="4">
  <tr>
    ...
    <td bgcolor="#CCCCCC"><strong>Course</strong></td>
    <td bgcolor="#CCCCCC"><strong>Attendance</strong></td>
