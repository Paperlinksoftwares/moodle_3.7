...
class grade_report_overview extends grade_report {
    ...
    public function __construct($userid, $gpr, $context) {
        ...
        $this->courses = enrol_get_users_courses($this->user->id, false, 'id, shortname, showgrades','startdate DESC');
        ...
        if($filename=="studentgrades")
        {
            foreach($this->courses as $key=>$val)
            {
                if($val->visible==0)
                {
                    $this->courses[$key]->visible='1';
                }
            }
        }
        ///
    ...
    public function print_table($return=false) {
        ...
        public function generate_table_data($activitylink = false, $studentcoursesonly = false) {
        global $CFG, $DB, $OUTPUT, $USER;
        $array = array();
        $arraycourse = array();
        if ($studentcoursesonly && count($this->studentcourseids) == 0) {
        ...
        if ($this->courses) {
            ...
            foreach ($this->courses as $course) {
                ...
                $data = array($coursename, grade_format_gradevalue($finalgrade, $course_item, true));
                ...
                $data[] = $course;
              //  $this->table->column_style_all('background-color', 'gold');
             //   echo '<pre>';
              //  print_r($data);
               // $this->table->add_data($data);
                $array[] = $data;
               // $arraycourse[] = $this->courses;
            }
            return $array;
