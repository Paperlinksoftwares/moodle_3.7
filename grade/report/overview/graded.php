 ...
 $report = new grade_report_overview($userid, $gpr, $context);
        ...
        groups_print_course_menu($course, $gpr->get_return_url('studentgrades.php?id='.$courseid, array('userid'=>0)));
		 ...
		 foreach($list as $key=>$val)
		 {
			 ...
			 $sql_instance = "SELECT `id` FROM {course_modules} as cm WHERE cm.`course` = '".$val->courseid."' 
			 AND cm.`instance` = '".$val->assignmentid."'";
			 $record_module = $DB->get_record_sql($sql_instance);
			 ...
			 $list_count[$val->assignmentid]['instanceid'] = $record_module->id;
			 ...
			 if($record_count->gradecount>0)
			 {
				 ...
				 foreach($record_grade_details as $key11=>$val11)
				 {
					 ...
					 if(count($submission_details)>0)
					 {
						 ...
						 if(strtolower($submission_details->status)=="submitted")
						 {
							 $list_count[$val->assignmentid]['submissiondetails'][$val11->userid]['timedifference'] = $list_count[$val->assignmentid]['gradingdate'][$k]-$submission_details->timecreated;
						 }
						 else if(strtolower($submission_details->status)=="new")
						 {
							 $list_count[$val->assignmentid]['submissiondetails'][$val11->userid]['timedifference'] = $list_count[$val->assignmentid]['gradingdate'][$k]-$list_count[$val->assignmentid]['assessmentcutoffdate'];
						 }
						 else if(strtolower($submission_details->status)=="reopened")
						 {
							 $list_count[$val->assignmentid]['submissiondetails'][$val11->userid]['timedifference'] = $list_count[$val->assignmentid]['gradingdate'][$k]-$list_count[$val->assignmentid]['assessmentcutoffdate'];
						 }
						 else
		 ...
		 $SESSION->list_count=$list_count;
		 $SESSION->stuidpost=$stuidpost;
		 //echo '<pre>';
		 //print_r($list_count);
...
if(count($list_count)>0) 
  {
	  ?>
<div style="float: right;"><input type="button" name="create_excel" id="create_excel" value=" Download Excel " class="myButton" onclick="javascript: window.open('createexcelgraded.php', '_blank')"></div>
  <br/><br/><?php } ?>
  <table id="customers">
  <tr>
  <th>Course</th>
...
window.onclick = function(event) {
  ...
  <?php } unset($sql_trainers); unset($ontime); unset($late); unset($d); unset($ip_array); unset($sql_ip);  } } else { echo '<tr><td colspan="8">&nbsp;&nbsp;<div style="color: red!important; font-weight: bold!important;" class="info-msg">No Records are Found!</div></td></tr>'; }
