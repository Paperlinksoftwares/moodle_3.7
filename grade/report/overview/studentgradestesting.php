...
<script>
    ...
    <?php
$PAGE->set_url(new moodle_url('/grade/report/overview/studentgradestesting.php', array('id' => $courseid, 'userid' => $userid)));
...
grade_regrade_final_grades_if_required($course);
    ...
    if (empty($userid)) { 
        ...
        groups_print_course_menu($course, $gpr->get_return_url('studentgradestesting.php?id='.$courseid, array('userid'=>0)));
...
<div class="accordion_container">
    <?php $course_name = array(); foreach($data_all as $key=>$val) { $course_name[] = $val[0]; ?>
    
        <div class="accordion_head"><input type="checkbox" name="select_course[]" id="select_course[]" value="<?php echo $val[2]->id; ?>" />&nbsp;<a href="<?php echo $CFG->wwwroot; ?>/course/view.php?id=<?php echo $val[2]->id; ?>" target="_blank" <?php if($val[2]->category==97 || $val[2]->category==98 || $val[2]->category==104 || $val[2]->category==111 || $val[2]->category==112) { ?> style="color: red!important; font-weight: bold!important;" <?php } ?>><?php echo $val[0]; ?></a>&nbsp;&nbsp;&nbsp;&nbsp;<span style="color: #6aad1d; font-weight: bold;"><?php //echo $val[1]; ?></span><span class="plusminus">+</span></div>
...
if($list_all_count>0) {
  ...
  <?php $cn = 0; foreach($arr as $key=>$val) { 
      ...
      $cn++; ?>
  <tr >
      <td><?php if($val['countsubmission']>1) { ?> <img src="<?php echo $CFG->wwwroot; ?>/pix/exclamation.png" border="0" /> <?php } ?> 
