...
if ( $xlsx = SimpleXLSX::parse($file)) {
	...
	for($k=1;$k<count($data_array);$k++)
	{
		...
		$new_array['module'][]=$data_array[$k][5];
		
		$sql_course = "SELECT `id`  FROM {course} WHERE `fullname` LIKE '%".$data_array[$k][5]."%'
		AND (`fullname` LIKE '%T".$_POST['term']."/".$_POST['year']."%' OR `fullname` LIKE '%T".$_POST['term'].",".$_POST['year']."%'
		OR `fullname` LIKE '%Term ".$_POST['term'].", ".$_POST['year']."%' ) ORDER BY `id`  DESC";
		$course_array = $DB->get_record_sql($sql_course);
		if(isset($course_array->id) && $course_array->id>0) {
		
		$new_array['courseid'][]=$course_array->id;
		} else
		{ $msg = "<div style='color: #fff;font-weight:bold; height:29px; width:100%;background-color: red;'>No Course / Unit Found under ".$data_array[$k][5]."
...
$extra_course1 = $DB->get_record_sql("SELECT a.id as courseid FROM `mdl_course` a LEFT JOIN 
	...
	a.fullname LIKE '%".$data_array[$k][5]."%' AND a.fullname NOT LIKE '%".$_POST['year']."%' LIMIT 0,1");
