...
if(!isset($_POST['studentid']) && isset($_POST['search']))
{
	...
	window.location.href='studentgradesnew.php.php';
	</script>
	<?php
}
?>
...
document.getElementById(qualification+s).checked=true;
    ...
    <?php
//$PAGE->set_url(new moodle_url('/grade/report/overview/studentgradesnew.php.php', array('id' => $courseid, 'userid' => $userid)));
$PAGE->set_url(new moodle_url('/grade/report/overview/studentgradesnew.php.php'));
...
if (has_capability('moodle/grade:viewall', $systemcontext)) {
 ...
 else if ($USER->id == 1187 || $USER->id==213) {
    // Ok - can view grades of this user - parent most probably.
    $access = true;
}
	...
	if($USER->id==74 || $USER->id==213 || $USER->id==1187)
{
    ...
    //    groups_print_course_menu($course, $gpr->get_return_url('studentgradesnew.php.php?id='.$courseid, array('userid'=>0)));
		groups_print_course_menu($course, $gpr->get_return_url('studentgradesnew.php.php'));
...
'; echo $rowc;	?>' 
		style="outline: 1px solid black; height: 15px; width:15px; border: none;" type="checkbox" name="select_course[]" id="<?php echo $mm.$rowc;	?>" value="<?php echo $val[2]->id; ?>" />&nbsp;&nbsp;<a href="<?php echo $CFG->wwwroot; ?>/course/view.php?id=<?php echo $val[2]->id; ?>" target="_blank" <?php if($val[2]->category==97 || $val[2]->category==98 || $val[2]->category==104 || $val[2]->category==111 || $val[2]->category==112 || $val[2]->category==116 || $val[2]->category==144 || $val[2]->category==164 || $val[2]->category==165 || $val[2]->category==166 || $val[2]->category==168 || $val[2]->category==195 || $val[2]->category==194) { ?> style="color: red!important; font-weight: bold!important;" <?php } ?> <?php if($val[2]->category==113 || $val[2]->category==115 || $val[2]->category==117 || $val[2]->category==128 || $val[2]->category==114 || $val[2]->category==137 || $val[2]->category==138) { ?> style="color: green!important; font-weight: bold!important;" <?php } ?> style="color: #000000; font-weight: bold!important;" ><?php echo $val[0]; ?></a>
...
//        }
	...
	as gg LEFT JOIN {grade_items} as gi ON gg.itemid = gi.id
		WHERE gi.courseid = "'.$val[2]->id.'" AND gg.userid = "'.$userid.'" AND gi.itemmodule ="quiz"';
...
<p>
 <table id="customers2">
  ...
  foreach($list_result_checklist as $key=>$val)
  {
	  ...
	  <td><?php if($val->timemodified!='') { echo @date("F j, Y, g:i a",$val->timemodified); } ?></td>
</tr>
	  <?php } ?> 
  ...
  foreach($list_result_quiz as $key2=>$val2)
  {
	  ...
	  <td><?php if($val2->timemodified!='') { echo @date("F j, Y, g:i a",$val2->timemodified); } ?></td>
</tr>
	  <?php } ?> 
