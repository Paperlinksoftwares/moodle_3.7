...
///UPDATE STAR RESULT SUBMIT ACTION

if(isset($_POST['submitform']))
{
	
	foreach($_POST['select_course'] as $key=>$val)
	{
		$u1 = $DB->execute("INSERT INTO `mdl_star_result_update`(`id`,`student_id`,`course_id`,`user_id`,`result`,`date_of_update`) 
		VALUES (NULL,'".$_POST['studentid']."', '".$val."' , '".$_POST['user_id']."' , 'Satisfactory' , '".@date('Y-m-d h:i:s')."')");
		
		$u2 = $DB->execute("UPDATE `mdl_grade_logs` SET `star_result_update_status` = '1' WHERE `course_id` = '".$val."' AND `student_id` = '".$_POST['studentid']."'");
		unset($u1);
		unset($u2);
	}
?>
<script>
window.location.href="starresultmanagement.php?studentid=<?php echo $_POST['studentid']; ?>";
</script>
<?php	
...
if(!isset($_POST['studentid']) && isset($_POST['search']))
{
	...
	window.location.href='starresultmanagement.php';
	</script>
	<?php
}
?>
<script>
    ...
    <?php
$PAGE->set_url(new moodle_url('/grade/report/overview/starresultmanagement.php', array('id' => $courseid, 'userid' => $userid)));
...
else if(isset($_GET['studentid']) && $_GET['studentid']>0)
{   
...
grade_regrade_final_grades_if_required($course);
    ...
    if (empty($userid)) { 
        ...
        groups_print_course_menu($course, $gpr->get_return_url('starresultmanagement.php?id='.$courseid, array('userid'=>0)));
        ...
        if ($user_selector) {
            ...
            if($if_student==0) { echo '<form id="search" name="search" autocomplete="off" action="" method="POST">
 ...
 <input id="studentid" type="hidden" name="studentid" value="" />
            </div></td><td>&nbsp;&nbsp;&nbsp;</td><td><input type="submit" name="search" value=" Search " style="margin: 0 0 0px 0px!important;" /></td></tr></table>
			<input type="hidden" name="search" id="search" value="1" /></form>'; }
        if(isset($userdetails) && $if_student==0 && ( isset($_POST['search']) || isset($_GET['studentid']))) { 
  ...
  <strong>Email</strong> : '.$userdetails->email.'<br><div style="height:6px;"></div>'.
        ...
        if($if_student==0) { echo '<div style="padding-left:10px;float:right; padding-top:5px;">'; ?> <a onclick="javascript: document.getElementById('f1').submit();" class="button-success" href="#">Update STAR Result</a></div>
		 <?php }  
...
// do not list all users
   ...
   <input id="studentname" type="text" name="studentname" placeholder="Type your Student Name" value="" />
 <input id="studentid" type="hidden" name="studentid" value="" />
           </div></td><td>&nbsp;&nbsp;&nbsp;</td><td><input class="button-success" type="submit" name="search" value=" Search " style="border: 0px solid #ccc!important;" /></td></tr></table>
        ...
        if ($currentgroup and !groups_is_member($currentgroup, $userid)) {
            ...
            if ($report->generate_table_data()) {
               ...
               // print_r($data_all);
                ?>
<br/><br/>
<form action="starresultmanagement.php" name="f1" id="f1" method="post">
<div class="accordion_container">
    <?php $course_name = array(); foreach($data_all as $key=>$val) { 
	//echo '<pre>';
	//print_r($val);
...
$enrolled = is_enrolled($context, $userdetails->id, '', true);
	...
	if($val[2]->category!=93 && $val[2]->category!=64) {
		 ...
		 $star_data_details = $DB->get_records_sql("SELECT *  FROM `mdl_star_result_update` WHERE `student_id` = '".$userdetails->id."' AND `course_id` = '".$val[2]->id."'");
      
	?>
	
	
	<div class="modal fade" id="myModal<?php echo $val[2]->id; ?>" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
				<?php foreach($star_data_details as $v=>$e)
		{
			//echo $e->date_of_update;
			echo @date("F j, Y, g:i a",strtotime($e->date_of_update));
			echo '<br/>';
		}
		?>
           </div>
        ...
        <div class="accordion_head" <?php if(count($star_data_details)>0) { ?> style="background-color: #8df4b8!important;" <?php } ?>  <?php echo $val[2]->category; ?>
		<?php 
	if($enrolled!=1 && !in_array($val[2]->category,$scv_cat_array) && !in_array($val[2]->category,$supervised_cat_array)) { ?>
		style="border: 2px dotted #ff0000!important; background-color: white!important;" <?php } ?>><input style="outline: 1px solid black; height: 15px; width:15px; border: none;" type="checkbox" name="select_course[]" id="select_course[]" value="<?php echo $val[2]->id; ?>" />&nbsp;&nbsp;<a href="<?php echo $CFG->wwwroot; ?>/course/view.php?id=<?php echo $val[2]->id; ?>" target="_blank" <?php if($val[2]->category==97 || $val[2]->category==98 || $val[2]->category==104 || $val[2]->category==111 || $val[2]->category==112 || $val[2]->category==116 || $val[2]->category==144 || $val[2]->category==164 || $val[2]->category==165 || $val[2]->category==166) { ?> style="color: red!important; font-weight: bold!important;" <?php } ?> <?php if($val[2]->category==113 || $val[2]->category==115 || $val[2]->category==117 || $val[2]->category==128 || $val[2]->category==114 || $val[2]->category==137 || $val[2]->category==138) { ?> style="color: green!important; font-weight: bold!important;" <?php } ?> style="color: #000000; font-weight: bold!important;" ><?php echo $val[0]; ?></a>
		&nbsp;&nbsp;&nbsp;&nbsp;<span style="color: #6aad1d; font-weight: bold;"><?php //echo $val[1]; ?></span>
	<?php if(count($star_data_details)>0) { ?>	<span>
		<button style="padding: 2px 4px 5px 4px!important; font-size: 14px!important; color: #fff!important;" type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal<?php echo $val[2]->id; ?>">View STAR Data Update History</button></span>
	<?php } ?>
...
<?php } else { echo '&nbsp;&nbsp;<span style="color: red!important; font-weight: bold!important;">Student did not open the submission link yet!</span>'; }
    ...
    <?php  unset($context); unset($star_data_details); } } ?>
  
</div>
    <input type="hidden" name="studentid" id="studentid" value="<?php echo $_REQUEST['studentid']; ?>" />
	<input type="hidden" name="studentname" id="studentname" value="<?php echo $userdetails->firstname." ".$userdetails->lastname; ?>" />
