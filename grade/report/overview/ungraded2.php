 ...
 $report = new grade_report_overview($userid, $gpr, $context);
	...
	ax.id as assignmentnameid , '
           ...
           . ' LEFT JOIN {assign} as ax ON  ax.id = z.assignment '
			...
			AND YEAR(FROM_UNIXTIME(mc.startdate)) = '.$year.' '.$clause1.' '.$clause2.' '.$clause3.'
			GROUP BY z.assignment ORDER BY z.timemodified '.$date_sort;
   ...
   /* $sql =    'SELECT z.id as rowid , z.status as status , z.userid , z.timemodified as activitydate , 
	...
	.' ORDER BY z.timemodified '.$date_sort;
...
$stnew = "[".$st."]"; 	
 ...
 <input id="studentid" type="hidden" name="studentid" value="" />
			...
			<form name="search2" autocomplete="off" action="ungraded2.php" method="POST">
			<table>
			<tr>
			<td></td>
  ...
  height: 114px;"><strong>Email</strong> : <a href="mailto: '.$userdetails->email.'">'.$userdetails->email.'</a><br><div style="height:6px;"></div>'.
        ...
        '<strong>Phone</strong>: '; if($other_details2->data!='') { echo $other_details2->data; } else { echo 'NA'; } echo '</div>'; }
           ?>
         <div style="float: right;"><a href="ungraded.php" class="button-success" style="font-size: 18px!important;">View All Records</a>
...
if($rolename=="student")
{
   ...
   { 
	...
	?>
<div>Total Records : <?php echo $total_c; ?><br/><br/>
         <table id="customers">
  <tr>
  <th>Student</th>
    <th>Assignment</th>
	<th>Course</th>
	<th><a href="ungraded2.php?sorting=<?php echo $sortval; ?>&year=<?php echo $year; ?>" style="color: white!important;">Start Date</a></th>
	<th>Trainer(s)</th>
    <th>Status</th>
     <th>
