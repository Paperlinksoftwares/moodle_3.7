...
if(!isset($_POST['studentid']) && isset($_POST['search']))
{
	...
	window.location.href='studentstudyplan.php';
	</script>
	<?php
}
?>
<script>
    ...
    <?php
$PAGE->set_url(new moodle_url('/grade/report/overview/studentstudyplan.php', array('id' => $courseid, 'userid' => $userid)));
...
grade_regrade_final_grades_if_required($course);
    ...
    if (empty($userid)) { 
        ...
        groups_print_course_menu($course, $gpr->get_return_url('studentstudyplan.php?id='.$courseid, array('userid'=>0)));
...
foreach($list_all_courses as $list_all_courses)
{
    ...
    $studentsstr = $studentsstr.'"'.$list_all_courses->coursename."|".$list_all_courses->courseid.'",';
}
$studentsstr = "[".$studentsstr."]";
   ...
   <input id="studentname" type="text" name="studentname" placeholder="Type your Course Name" value="" />
...
$studentsstr = "[".$studentsstr."]";
 ...
 <input id="studentid" type="hidden" name="studentid" value="" />
  ...
  <strong>Email</strong> : <a href="mailto: '.$userdetails->email.'">'.$userdetails->email.'</a><br><div style="height:6px;"></div>'.
        ...
        if($if_student==0) { echo '<div style="padding-left:10px;float:right; padding-top:5px;">'; ?> <a onclick="javascript: sendSMS('<?php echo $other_details2->data; ?>','<?php echo @$userdetails->firstname.' '.@$userdetails->lastname; ?>');" class="button-success" href="#">Send Study Plan</a></div></div> <?php } }
	...
	$sql_all_list =		'SELECT u.id as userid , u.lastname AS lastname , u.firstname as firstname , r.name,r.id
...
$data_all = $DB->get_records_sql($sql_all_list);
...
<div class="accordion_container">
	...
	?>
        ...
        <div class="accordion_head"  <?php echo $val->userid; ?>
		><input style="outline: 1px solid black; height: 15px; width:15px; border: none;" type="checkbox" name="select_course[]" id="select_course[]" value="<?php echo $val->userid; ?>" />&nbsp;&nbsp;<a href="<?php echo $CFG->wwwroot; ?>/course/view.php?id=<?php echo $val->userid; ?>" target="_blank"  style="color: #000000; font-weight: bold!important;" ><?php echo $val->firstname." ".$val->lastname; ?></a>&nbsp;&nbsp;&nbsp;&nbsp;<span style="color: #6aad1d; font-weight: bold;"><?php //echo $val[1]; ?></span><span class="plusminus">+</span></div>
