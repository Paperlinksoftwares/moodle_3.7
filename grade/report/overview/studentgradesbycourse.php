...
$arrnew = $DB->get_records_sql("SELECT ue.id, c.id AS courseid, c.fullname as course_name, u.id AS userid , u.firstname as fname FROM mdl_user u 
...
WHERE e.status = 0 AND u.suspended = 0 AND u.deleted = 0 AND (ue.timeend = 0 OR ue.timeend > NOW()) AND ue.status = 0 LIMIT 0,50");
...
echo '<link rel="stylesheet" href="https://unpkg.com/purecss@2.0.3/build/pure-min.css" integrity="sha384-cg6SkqEOCV1NbJoCu11+bm0NvBRc8IYLRGXkmNrqUBfTjmMYwNKPWBTIKyw9mHNJ" crossorigin="anonymous">
';
echo '<style>
   ...
   <select onchange="javascript: generateReport(this.value);">
        ...
        if ($currentgroup and !groups_is_member($currentgroup, $userid)) {
            ...
            if ($report->generate_table_data()) {
                ...
                ?>
<br/><br/>
<table class="pure-table" style="width:100%!important;">
<thead>
<tr>
<th>Course</th>
<th>Student</th>
...
if($list_all_count>0) {
    ...
    //print_r($arr);
          ...
          $cn = 0; foreach($arr as $key=>$val) { if(strtolower($val['result'])!='satisfactory' || strtolower($val['result'])!='competent' || strtolower($val['result'])!='outstanding')
  { $pass = false; } }
        ?>

      <td><?php if($pass==true) { echo 'Passed'; } else { echo 'Failed'; } ?></td>
...
?>
<td>&nbsp;&nbsp;<span style="color: red!important; font-weight: bold!important;">NA</span></td>' <?php }
