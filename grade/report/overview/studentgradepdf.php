...
grade_regrade_final_grades_if_required($course);
 ...
 <input id="studentid" type="hidden" name="studentid" value="" />
        ...
        if(isset($userdetails)) { echo '<br/>Search results of <a href="'.$CFG->wwwroot.'/user/profile.php?id='.$userdetails->id.'" target="_blank"><strong>'.@$userdetails->firstname.' '.@$userdetails->lastname.'</strong></a> - <div style="float:right;"><a class="button-success" href="mailto: '.@$userdetails->email.'">Send email to '.@$userdetails->firstname.' '.@$userdetails->lastname.'</a></div>';  } } 
 ...
 <input id="studentid" type="hidden" name="studentid" value="" />
        ...
        if ($currentgroup and !groups_is_member($currentgroup, $userid)) {
            ...
            if ($report->generate_table_data()) {
               ...
               // print_r($data_all);
                ?>
<br/><br/><div class="accordion_container">
    <?php foreach($data_all as $key=>$val) { ?>
    <div class="accordion_head"><a href="<?php echo $CFG->wwwroot; ?>/course/view.php?id=<?php echo $val[2]->id; ?>" target="_blank" <?php if($val[2]->category==97 || $val[2]->category==98 || $val[2]->category==104 || $val[2]->category==111 || $val[2]->category==112) { ?> style="color: red!important; font-weight: bold!important;" <?php } ?>><?php echo $val[0]; ?></a>&nbsp;&nbsp;&nbsp;&nbsp;<span style="color: #6aad1d; font-weight: bold;"><?php //echo $val[1]; ?></span><span class="plusminus">+</span></div>
...
//        }
    ...
    $sql_all =   'SELECT z.id as rowid , z.status as status , z.timemodified as activitydate , gg.usermodified , gg.feedback , gg.timecreated as feedbackposted1 , gg.timemodified as feedbackposted2 , z.userid , y.firstname , y.lastname , ag.grade as recorded_grade , gi.iteminstance as assignmentid , ag.timemodified, ax.name as assignmentname, ax.id as assignmentnameid , '
           ...
           . ' LEFT JOIN {assign} as ax ON  ax.id = z.assignment '
            ...
            .' WHERE z.userid = '.$userid.' AND ax.course = '.$val[2]->id.' and z.timemodified = (SELECT max(`timemodified`) FROM {assign_submission} WHERE `assignment` = ax.id AND `userid` = gg.userid) GROUP BY z.assignment , z.userid ';
...
<!--    <th>Minimum Grade</th>
  ...
  <?php $cn = 0; foreach($arr as $key=>$val) { 
      ...
      $cn++; ?>
  <tr>
      <td><?php if($if_student==0) { ?><a href="<?php echo $CFG->wwwroot; ?>/mod/assign/view.php?id=<?php echo $val['moduleid']; ?>&action=grading" target="_blank"><?php echo $val['assignmentname']; ?></a><?php } else { ?><?php echo $val['assignmentname']; ?><?php } ?></td>
   ...
   <!-- <td><?php //echo $val['grademin']; ?></td>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
       ...
       <button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal<?php echo $val['rowid'].$cn; ?>">View</button></td>
  </tr>
  <?php } ?>
