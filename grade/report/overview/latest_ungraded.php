...
<script>
    ...
    $sql_all =   'SELECT z.id as rowid , z.status as status , z.timemodified as activitydate , mc.fullname , mc.id as courseid , mc.startdate , 
	...
	ax.id as assignmentnameid , '
           ...
           . ' LEFT JOIN {assign} as ax ON  ax.id = z.assignment '
			...
			AND YEAR(FROM_UNIXTIME(mc.startdate)) > 2017
			GROUP BY z.assignment ORDER BY FIELD(z.status,"submitted","draft","reopened","new")';
   ...
   /* $sql =    'SELECT z.id as rowid , z.status as status , z.userid , z.timemodified as activitydate , 
	...
	ax.cutoffdate as assessmentcutoffdate, ax.gradingduedate as gradingduedate , mc.fullname , mc.id as courseid , 
...
foreach($list as $list)
{
    ...
    else
    {
			...
			if($scale_text == 'NA / Not yet graded' || $scale_text=="Not Satisfactory")
			{
            $SESSION->arr[]  = array('baseurl'=>$CFG->wwwroot,'rowid'=>$list->rowid,'coursemoduleid'=>$list->coursemoduleid,'userid'=>$list->userid, 'name'=>$list->firstname.' '.$list->lastname, 'fullname' => $list->fullname , 'courseid' => $list->courseid , 
...
else if($list->scaleid=='' && $list->gradetype==1)
{
    if(@$contextid->id!='' && @$contextid->id>0)
    {
        ...
        if($list_all_stu->roleid!='' && $list_all_stu->roleid==5)
        {
			...
			if($scale_text == 'NA / Not yet graded' || $scale_text=="Not Satisfactory")
			{
            $SESSION->arr[]  = array('baseurl'=>$CFG->wwwroot,'rowid'=>$list->rowid,'coursemoduleid'=>$list->coursemoduleid,'userid'=>$list->userid, 'name'=>$list->firstname.' '.$list->lastname, 
...
$cc = 0;
foreach($trainers_list as $key=>$val[$i])
{
	if(($ct-$cc)>1)
	{
	$st = $st . '"'.$valu[$i]->fname.' '.$valu[$i]->lname.'|'.$valu[$i]->userid.'",';
	}
	else 
	{
		$st = $st . '"'.$valu[$i]->fname.' '.$valu[$i]->lname.'|'.$valu[$i]->userid.'"';
...
$stnew = "[".$st."]"; 	
 ...
 <input id="studentid" type="hidden" name="studentid" value="" />
            </div></td><td>&nbsp;&nbsp;&nbsp;</td><td><input type="submit" name="search" value=" Search " style="margin: 0 0 0px 0px!important;" /></td>
			<td>&nbsp;&nbsp;<input type="button" name="reset" value=" Show All " style="margin: 0 0 0px 0px!important;" onclick="window.location.href='ungraded.php';" /></td></tr></table></form>
...
$current_timestamp=time();
  $counter = 0;
  $cn = 0; $ct = 0; $rate_arr=array(); for($i=$pageLimit;$i<($pageLimit+$setLimit);$i++) { 

//	if($val[$i]['result'] == 'NA / Not yet graded' || ( $val[$i]['countsubmission']>1 && $val[$i]['result']=="Not Satisfactory"))
	if($SESSION->arr[$i]['result'] == 'NA / Not yet graded' || $SESSION->arr[$i]['result']=="Not Satisfactory")
	{
  $context = context_course::instance($SESSION->arr[$i]['courseid']);
$roles = get_user_roles($context, $SESSION->arr[$i]['userid'], true);
$role = key($roles);
...
WHERE c.id = '".$SESSION->arr[$i]['courseid']."'";
...
if((isset($_POST['search']) && in_array($_POST['studentid'],$arr_check_tr)==true) || !isset($_POST['search']))
{



if(($SESSION->arr[$i]['result']=='Not Satisfactory' || $SESSION->arr[$i]['result']=='NA / Not yet graded')  && ( strpos(strtolower($SESSION->arr[$i]['fullname']),"scv")==false && strpos(strtolower($SESSION->arr[$i]['fullname']),"supervised")==false)) { $color = "#c4ebed"; }
...
if(($SESSION->arr[$i]['result']=='Not Satisfactory' || $SESSION->arr[$i]['result']=='NA / Not yet graded') && ( strpos(strtolower($SESSION->arr[$i]['fullname']),"scv")==true && strpos(strtolower($SESSION->arr[$i]['fullname']),"supervised")==false)) { $color = "#bdf2b8"; }
if(($SESSION->arr[$i]['result']=='Not Satisfactory' || $SESSION->arr[$i]['result']=='NA / Not yet graded') && ( strpos(strtolower($SESSION->arr[$i]['fullname']),"scv")==false && strpos(strtolower($SESSION->arr[$i]['fullname']),"supervised")==true)) { $color = "#f2dea4"; }

if(strtotime($SESSION->arr[$i]['activitydate'])>0 && strtotime($SESSION->arr[$i]['activitydate'])>strtotime($SESSION->arr[$i]['timemodified']) && strtolower($SESSION->arr[$i]['status'])!="new")
{
  $timestamp_diff_grading = ( $current_timestamp - strtotime($SESSION->arr[$i]['activitydate']));
}
...
*/
  ...
  <td><?php echo $SESSION->arr[$i]['name']; ?>
      <td><?php if($SESSION->arr[$i]['countsubmission']>1) { ?> <img style="height:23px; width: 20px;" alt="It has multiple submission! Please check." title="It has multiple submission! Please check." src="<?php echo $CFG->wwwroot; ?>/pix/exclamation.png" border="0" /> <?php } ?> 
	 

     <?php if($if_student==0) { ?><a href="<?php echo $CFG->wwwroot; ?>/mod/assign/view.php?id=<?php echo $SESSION->arr[$i]['moduleid']; ?>&action=grading" target="_blank"><?php echo $SESSION->arr[$i]['assignmentname']; ?></a><?php } else { ?><?php echo $SESSION->arr[$i]['assignmentname']; ?><?php } ?></td>
    <td><?php echo $SESSION->arr[$i]['fullname']; ?></td>
	<td><?php echo $SESSION->arr[$i]['coursestartdate']; ?></td>
	...
	<td><?php if(strtolower($SESSION->arr[$i]['status'])=="new") { echo 'No Submission'; } else { echo $SESSION->arr[$i]['status']; } ?></td>
   
	<td><?php echo $SESSION->arr[$i]['activitydate']; ?></td>
    <td><?php echo $SESSION->arr[$i]['timemodified']; ?></td> 
...
?></td>
   <td>
   <?php if((strtolower($SESSION->arr[$i]['status'])=="new" && strpos(strtolower($SESSION->arr[$i]['fullname']),"scv")==false && strpos(strtolower($SESSION->arr[$i]['fullname']),"supervised")==false
   && (($SESSION->arr[$i]['duedate']<$current_timestamp && $SESSION->arr[$i]['assessmentcutofffdate']==0) || ($SESSION->arr[$i]['duedate']<$current_timestamp && $SESSION->arr[$i]['assessmentcutofffdate']<$current_timestamp)))
   || ( strtolower($SESSION->arr[$i]['status'])=='reopened' ) || ( strtotime($SESSION->arr[$i]['activitydate'])>strtotime($SESSION->arr[$i]['timemodified']) && strtolower($SESSION->arr[$i]['status'])!='new')) { ?>
   
   <a target="_blank" href="<?php echo $CFG->wwwroot; ?>/mod/assign/view.php?id=<?php echo $SESSION->arr[$i]['moduleid']; ?>&rownum=0&action=grader&userid=<?php echo $SESSION->arr[$i]['userid']; ?>" class="button-success">GRADE</a>
...
<?php } ?>
	   ...
	   <?php if(strpos(strtolower($SESSION->arr[$i]['fullname']),"scv")==false && strpos(strtolower($SESSION->arr[$i]['fullname']),"supervised")==false && $SESSION->arr[$i]['result']=="Not Satisfactory")
	    { ?>
   ...
   else if(strtolower($SESSION->arr[$i]['status'])=='draft' && strtotime($SESSION->arr[$i]['timemodified'])>strtotime($SESSION->arr[$i]['activitydate']))
...
<tr style="border: 3pt solid red;">
   <td colspan="12">&nbsp;Please contact to the student.</td>
	   ...
	   <?php } 
	    ...
	    else if(strtolower($SESSION->arr[$i]['status'])=='reopened' && strtotime($SESSION->arr[$i]['activitydate'])>strtotime($SESSION->arr[$i]['timemodified']))
  ...
  <?php unset($color); }  unset($rolename); unset($context); unset($roles); unset($role); unset($trainers); unset($sql_trainers); unset($timestamp_diff_grading); } } } ?>
