...
if(isset($_REQUEST['stuid']))
{
	...
	$clause1 = " AND z.userid = '".$stuid."'";
}
else
{
	$stuid = '';
	$clause1 = " AND z.userid > 0";
}
 ...
 $report = new grade_report_overview($userid, $gpr, $context);
	...
	ax.id as assignmentnameid , '
           ...
           . ' LEFT JOIN {assign} as ax ON  ax.id = z.assignment '
			...
			AND YEAR(FROM_UNIXTIME(mc.startdate)) = '.$year.' '.$clause1.'
	...
	ag.timemodified, ax.name as assignmentname,  ax.id as assignmentnameid , '
           ...
           . ' LEFT JOIN {assign} as ax ON  ax.id = z.assignment '
			...
			.' LEFT JOIN {course_modules} as cm ON cm.course = ax.course AND cm.instance = gi.iteminstance AND cm.module = "29"'  
               	.' WHERE YEAR(FROM_UNIXTIME(mc.startdate)) = '.$year.' '.$clause1.' '
...
$stnew2 = "[".$st2."]"; 	
 ...
 <input id="stuid" type="hidden" name="stuid" value="" />
   ...
   <td>&nbsp;&nbsp;</td>
			...
			<td>&nbsp;</td>
			<td>&nbsp;&nbsp;<input type="button" name="reset" value=" Show All " style="margin: 0 0 0px 0px!important;" onclick="window.location.href='ungraded3.php';" /></td>
		</tr></table></form>
  ...
  <th>Student</th>
    <th>Assignment</th>
	<th>Course</th>
	<th><a href="ungraded3.php?sorting=<?php echo $sortval; ?>&year=<?php echo $year; ?>" style="color: white!important;">Start Date</a></th>
	<th>Trainer(s)</th>
    <th>Status</th>
    <th>Last Updated</th>
   <th>Graded On</th>
...
if((isset($_POST['search']) && in_array($_POST['studentid'],$arr_check_tr)==true) || (!isset($_POST['search']) && $if_student==0) || ( $if_student==1 && in_array($USER->id,$arr_check_tr)==true) || (isset($_POST['search']) && $_POST['studentid']=='' && $if_student==0))
{
...
?></td>
  ...
  <?php  }  unset($rolename); unset($context); unset($roles); unset($role); unset($trainers); unset($sql_trainers); unset($timestamp_diff_grading); } } } } ?>
  
  

  <?php } else { echo '&nbsp;&nbsp;<tr><td colspan="11"><span style="color: red!important; font-weight: bold!important;">No Records Found OR Student did not open the submission link yet!</span></td></tr>'; } 
