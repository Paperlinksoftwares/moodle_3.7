	...
	function selectAllCheckbox(qualification,countall)
	{
		...
		if(countall>0)
		{
			for(var i=1;i<countall;i++)
			{
				...
				//document.getElementsByClassName(qualification+'|'+i).setAttribute('checked', 'checked');
				alert('ok');
			}
		}
	}
            ...
            if ($report->generate_table_data()) {
               ...
               // print_r($data_all);
				...
				/*	if(stristr($c_name_arr[0],'DT')==true)
					...
					{
						$data_all_new['CPD - Certificate in Painting and Decorating'][]=$vv;
					}
...
<div class="accordion_container">
    <?php $course_name = array(); foreach($data_all_new as $mm=>$pp) 
    ...
    color: white;
    padding: 3px 10px 10px 10px; margin-top: 11px;"><input type="checkbox" onclick="javascript: selectAllCheckbox('<?php echo $mm; ?>','<?php echo count($pp); ?>');" name="selectall" value="" />&nbsp;&nbsp;<?php echo $mm; ?></div>
...
'; echo $rowc;	?>' 
		style="outline: 1px solid black; height: 15px; width:15px; border: none;" type="checkbox" name="select_course[]" id="select_course[]" value="<?php echo $val[2]->id; ?>" />&nbsp;&nbsp;<a href="<?php echo $CFG->wwwroot; ?>/course/view.php?id=<?php echo $val[2]->id; ?>" target="_blank" <?php if($val[2]->category==97 || $val[2]->category==98 || $val[2]->category==104 || $val[2]->category==111 || $val[2]->category==112 || $val[2]->category==116 || $val[2]->category==144 || $val[2]->category==164 || $val[2]->category==165 || $val[2]->category==166 || $val[2]->category==168) { ?> style="color: red!important; font-weight: bold!important;" <?php } ?> <?php if($val[2]->category==113 || $val[2]->category==115 || $val[2]->category==117 || $val[2]->category==128 || $val[2]->category==114 || $val[2]->category==137 || $val[2]->category==138) { ?> style="color: green!important; font-weight: bold!important;" <?php } ?> style="color: #000000; font-weight: bold!important;" ><?php echo $val[0]; ?></a>&nbsp;&nbsp;&nbsp;&nbsp;<span style="color: #6aad1d; font-weight: bold;"><?php //echo $val[1]; ?></span><span class="plusminus">+</span></div>
...
<?php } else { echo '&nbsp;&nbsp;<span style="color: red!important; font-weight: bold!important;">Student did not open the submission link yet!</span>'; }
         ...
         ?>
			...
			<?php  unset($context);  } $rowc++; } ?> </div> <?php } ?>
