...
<?php
$sql_begin = "SELECT d.id , d.date_of_view , d.year , d.term , u.firstname, u.lastname , u.email, u.id as userid FROM ".$SETTINGS["data_table"]." 
...
if ($_REQUEST["from"]<>'' and $_REQUEST["to"]<>'') {
	$sql = $sql_begin." WHERE d.date_of_view >= '".@date("Y-m-d", strtotime($_REQUEST["from"]))."' AND d.date_of_view <= '".@date("Y-m-d", strtotime($_REQUEST["to"]))."'".$search_string;
	$_SESSION['from']=$_REQUEST["from"];
	$_SESSION['to']=$_REQUEST["to"];
} else if ($_REQUEST["from"]<>'') {
	$sql = $sql_begin." WHERE d.date_of_view >= '".@date("Y-m-d", strtotime($_REQUEST["from"]))."'".$search_string;
	$_SESSION['from']=$_REQUEST["from"];
} else if ($_REQUEST["to"]<>'') {
	$sql = $sql_begin." WHERE d.date_of_view <= '".@date("Y-m-d", strtotime($_REQUEST["to"]))."'".$search_string;
	$_SESSION['to']=$_REQUEST["to"];
} else {
...
<table width="100%" border="1" cellspacing="0" cellpadding="4">
  <tr>
    ...
    <td bgcolor="#CCCCCC"><strong>Date of View</strong></td>
  </tr>
  <?php
...
?>
  <tr>
   ...
   <td><?php echo $row["term"]; ?></td>
    ...
    <td><?php echo @date("l jS \of F Y h:i A", strtotime($row["date_of_view"])); ?></td>
      </tr>
<?php
