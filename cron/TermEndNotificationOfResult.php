...
$hours = 3000; // hours amount (integer) you want to add
...
$sql_grades = "SELECT a.id , a.grader, a.grade, a.assignment, a.userid, b.course , b.name as assignment_name , c.fullname , u.email, 
u.phone1, u.phone2 , u.firstname , u.lastname , u.username FROM `mdl_assign_grades` as a 
LEFT JOIN `mdl_user` as u ON a.userid = u.id
LEFT JOIN `mdl_assign` as b ON a.assignment = b.id
LEFT JOIN `mdl_course` as c ON b.course = c.id
 WHERE a.`timemodified` >= '".$from_timestamp."' AND a.`timemodified` <= '".$to_timestamp."' AND a.`grader` > 0 LIMIT 0,1"; 

$result = mysqli_query($conn, $sql_grades);

$subject = "ACCIT | Notification of Grading - TERM 1 | ".@date('Y');
...
while($row_result = mysqli_fetch_assoc($result))
{
	if($row_result['email']!='')
			...
			/* $arr['name'][] = $row_result['firstname'].' '.$row_result['lastname'];
			 $arr['student_no'][] = $row_result['username'];
			 $arr['grade'][] = $final_result;
			 */
			 $messageHtml = $messageHtml."<br/>".$row_result['firstname'].' '.$row_result['lastname']."|".$row_result['username']."|".$row_result['assignment_name']."|".$final_result;

	//	@mail($row_result['email'],$subject,$messageHtml,$headers);
//	@mail('debraj.a@accit.nsw.edu.au',$subject,$messageHtml,$headers);
...
@mail('jeeva@accit.nsw.edu.au',$subject,$messageHtml,$headers);
die;
?>