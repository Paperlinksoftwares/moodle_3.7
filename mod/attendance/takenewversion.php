...
<script> 
	...
	//NEW METHOD
	function setAllCheckedNew(userid,val)
    {
		if(val==0)
		{
			 document.getElementById('atthr'+userid+'-1').checked=false;
             document.getElementById('atthr'+userid+'-1').setAttribute('disabled', 'disabled');
			 document.getElementById('atthr'+userid+'-2').checked=false;
             document.getElementById('atthr'+userid+'-2').setAttribute('disabled', 'disabled');
		}
		else if(val==1)
		{
			document.getElementById('atthr'+userid+'-1').checked=true;
            // document.getElementById('atthr'+userid+'-1').setAttribute('disabled', 'disabled');
			 document.getElementById('atthr'+userid+'-2').checked=true;
           //  document.getElementById('atthr'+userid+'-2').setAttribute('disabled', 'disabled');
		}
		else
		{
...
$duration_hours_first_half=intval($duration_hours/2); 
$duration_hours_extra_first_half = ($duration_hours/2) - $duration_hours_first_half;
$duration_hours_extra = $total_duration-$duration_hours;
$duration_hours_second_half = $duration_hours_first_half + $duration_hours_extra_first_half + $duration_hours_extra;
...
$renderer = $PAGE->get_renderer('gradereport_overview');
 ...
 <input id="studentid" type="hidden" name="studentid" value="" />
	...
	padding:5px 19px;
	text-decoration:none;
	text-shadow:0px 1px 0px #3d768a;" type="submit" name="search" value=" Reset " onclick="javascript: document.getElementById('studentname').value='';" style="margin: 0 0 0px 0px!important;" /></td></tr></table></form> 
<?php
...
<td style="border-right: 1px solid #ccc; width: 90px;">Full Present</td><td style="border-right: 1px solid #ccc; width: 90px;">Full Absent</td>';
...
<label class="container"><input type="radio" name="full_'.$users->id.'" id="full_'.$users->id.'" value="1" onchange=\'javascript: setAllCheckedNew('.$users->id.',1);\' '.$check_full2.'  /><span class="checkmark2"></span>
</label></td>';
echo '<td style="border-right: 1px solid #ccc; width: 90px; text-align: center;"><label class="container"><input type="radio" name="full_'.$users->id.'" id="full_'.$users->id.'" value="0" onchange=\'javascript: setAllCheckedNew('.$users->id.',0);\' '.$check_full.'  /><span class="checkmark2"></span></td>';

	if(in_array($k,explode(",",@$hrs_array->hours))==true) { $checked = "checked"; } else { $checked = ''; }
echo '<td style="border-right: 1px solid #ccc; width: 90px; text-align: center;">
<label class="container">
<input type="checkbox" name="atthr'.$users->id.'-1" id="atthr'.$users->id.'-1" value="'.$duration_hours_first_half.'" onchange=\'javascript: countAbsHr('.$users->id.',1,0);\' '.$checked.' '.$dis.'   /><span class="checkmark"></span>
</label></td>';

echo '<td style="border-right: 1px solid #ccc; width: 90px; text-align: center;">
<label class="container">
<input type="checkbox" name="atthr'.$users->id.'-2" id="atthr'.$users->id.'-2" value="'.$duration_hours_second_half.'" onchange=\'javascript: countAbsHr('.$users->id.',2,0);\' '.$checked.' '.$dis.'   /><span class="checkmark"></span>
</label></td>'; 
