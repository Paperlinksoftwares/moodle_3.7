...
@keyframes blinkingText{
    ...
    100%{   color: #ff0000;    }
            ...
            $this->add_table_row_tuple($t, get_string('duedate', 'assign'),
                                       userdate($duedate));
  //$this->add_table_row_tuple($t, "<b><span class='blinking' style='font-size: 19px!important;'>".get_string('duedate', 'assign')."</span></b>",
            ...
            if ($duedate < $time) {
                ...
                if ($cutoffdate) {
                    if ($cutoffdate > $time) {
                        $late = "<b><span class='blinking' style='font-size: 19px!important;'>".get_string('latesubmissionsaccepted', 'assign', userdate($summary->cutoffdate))."</span></b>";
                    } else {
                        $late = get_string('nomoresubmissionsaccepted', 'assign');
                    }
                    $this->add_table_row_tuple($t, "<b><span class='blinking' style='font-size: 19px!important;'>Submission Cut-Off</span></b>", $late);
                }
            }
...
.blinking{
    animation:blinkingText 2.9s infinite;
}
@keyframes blinkingText{
    0%{     color: #ff0000;    }
    49%{    color: #ff0000; }
    ...
    100%{   color: #ff0000;    }
			...
			//if ($status->view == assign_submission_status::GRADER_VIEW) {
                if ($status->cutoffdate && $status->extensionduedate=='') {
                    // Cut off date.
                    $row = new html_table_row();
                    $cell1 = new html_table_cell("<b><span class='blinking' style='font-size: 19px!important;'>Submission Cut-off</span></b>");
                    $cell2 = new html_table_cell("<b><span class='blinking' style='font-size: 19px!important;'>".userdate($status->cutoffdate)."</span></b>");
                    $row->cells = array($cell1, $cell2);
                    $t->data[] = $row;
                }
				else
				{
					$row = new html_table_row();
                    $cell1 = new html_table_cell("Submission Cut-off");
                    $cell2 = new html_table_cell(userdate($status->cutoffdate));
                    $row->cells = array($cell1, $cell2);
                    $t->data[] = $row;
				}
       ...
       /*     if ($status->extensionduedate) {
			...
			*/
			...
			if ($status->extensionduedate) {
                // Extension date.
                $row = new html_table_row();
                $cell1 = new html_table_cell("<b><span class='blinking' style='font-size: 19px!important;'>".get_string('extensionduedate', 'assign')."</span></b>");
                $cell2 = new html_table_cell("<b><span class='blinking' style='font-size: 19px!important;'>".userdate($status->extensionduedate)."</span></b>");
                $row->cells = array($cell1, $cell2);
                $t->data[] = $row;
                $duedate = $status->extensionduedate;
            }
...
//echo $status->courseid;
//echo $submission->userid;
// ADDED //
...
global $DB;
$arrnew = $DB->get_record_sql("SELECT e.id, e.courseid , ue.userid , ue.enrolid , ue.timeend FROM mdl_enrol e 
JOIN mdl_user_enrolments ue ON ue.enrolid = e.id 
WHERE e.courseid = '".$status->courseid."' AND ue.userid = '".$submission->userid."'");

if (!empty($arrnew->timeend)) {
            $row = new html_table_row();
            $cell1 = new html_table_cell("<b><span class='blinking' style='font-size: 19px!important;'>Cut-Off date</span></b>");
            $cell2 = new html_table_cell("<b><span class='blinking' style='font-size: 19px!important;'>".date('dS F Y h:i:s A', $arrnew->timeend)."</span></b>");
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;
        }
        ...
        if ($status->view == assign_submission_status::STUDENT_VIEW) {
            if ($status->canedit) {
                if (!$submission || $submission->status == ASSIGN_SUBMISSION_STATUS_NEW) {
				  ...
				  /*UPDATED THE CODE JUST TO HIDE THE BUTTON FOR ATTEMPT TO EDIT PREVIOUS SUBMISSION IN ORDER TO WORK ANNOTATE PDF WORK PROPERLY*/
	...
	//	echo $table->get_rows_per_page(); die;
