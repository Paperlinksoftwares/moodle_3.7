	 ...
	 foreach($list_file as $t=>$e)
		{
			...
			$ext = end($arr);
			if(strtolower($ext)=='ppt' || strtolower($ext)=='pptx' || strtolower($ext)=='pps' || strtolower($ext)=='ppsx' || strtolower($ext)=='pub')
			{
				$has_ppt = 1;
			}
			if(strtolower($ext)=='xlsm' || strtolower($ext)=='xlsx' || strtolower($ext)=='xls')
			{
				$has_excel = 1;
			}
			unset($ext);
...
if($has_ppt==0 && (in_array(".ppt",$filetype_arr)==true || in_array(".PPT",$filetype_arr)==true || in_array(".pps",$filetype_arr)==true || in_array(".PPS",$filetype_arr)==true
|| in_array(".ppsx",$filetype_arr)==true || in_array(".PPSX",$filetype_arr)==true || in_array(".pptx",$filetype_arr)==true || in_array(".PPTX",$filetype_arr)==true || in_array(".pub",$filetype_arr)==true))
		{
			?>
			<style>
			.myButton {
	background:linear-gradient(to bottom, #2347a3 5%, #647de3 100%);
	...
	&:hover {
	background:linear-gradient(to bottom, #647de3 5%, #2347a3 100%);
	background-color:#647de3;
}
 &:active {
	position:relative;
	top:1px;
  ...
  $( function() {
    $( "#dialog-confirm" ).dialog({
      ...
      modal: true
			...
			<div id="dialog-confirm" title="" style="width: 40%; font-size: 17px; font-family: Helvetica, sans-serif;
  ...
  padding: 8px 12px;
  background-image: linear-gradient(180deg, #fff, #ddd 40%, #ccc); height: 138px;">
  <p><strong>This assessment requires you to submit Powerpoint presentation file(s). You haven't attached the file(s) yet. This may incur not satisfactory course progress.</strong>
  </p>
  <p style="padding-top: 7px;">
  <a href="http://localhost/accit-moodle/accit/mod/assign/view.php?id=<?php echo $this->get_course_module()->id; ?>" class="myButton">Okay, I understand</a>
  </p></div>
...
if($has_excel==0 && (in_array(".xlsm",$filetype_arr)==true || in_array(".XLSM",$filetype_arr)==true || in_array(".xlsx",$filetype_arr)==true || in_array(".XLSX",$filetype_arr)==true
|| in_array(".xls",$filetype_arr)==true || in_array(".XLS",$filetype_arr)==true))
		{
			?>
			<style>
			.myButton {
 ...
 &:active {
  ...
  padding: 8px 12px;
  background-image: linear-gradient(180deg, #fff, #ddd 40%, #ccc); height: 138px;">
  <p><strong>This assessment requires you to submit Excel file(s). You haven't attached the file(s) yet. This may incur not satisfactory course progress.</strong>
  </p>
  <p style="padding-top: 7px;">
  <a href="http://localhost/accit-moodle/accit/mod/assign/view.php?id=<?php echo $this->get_course_module()->id; ?>" class="myButton">Okay, I understand</a>
  </p></div>
