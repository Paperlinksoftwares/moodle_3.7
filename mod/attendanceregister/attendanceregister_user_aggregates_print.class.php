 ...
 * @author nicus
 */
class attendanceregister_user_aggregates_print  {
     ...
     */
    public function __construct($register, $userId, attendanceregister_user_sessions_print $userSessions) {
        global $DB;
    ...
    public function html_table() {
       ...
       // print_r($this->user);
        ...
        $table->attributes['class'] .= ' gridtable';
        @$table->attributes['style'] .= 'width: 60%!important;';
        ...
        $labelCell->colspan = 2;
        $labelCell->text = get_string('full_name', 'attendanceregister');
        $row->cells[] = $labelCell;
        $valueCell = new html_table_cell();
        $valueCell->text = $this->user->firstname." ".$this->user->lastname." | ".$this->user->username;
        $row->cells[] = $valueCell;
        $table->data[] = $row;
