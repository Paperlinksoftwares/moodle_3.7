...
require_once("attendanceregister_tracked_users_print.class.php");
require_once("attendanceregister_user_sessions_print.class.php");
require_once("attendanceregister_user_aggregates_print.class.php");
...
//BELOW FUNCTION ADDED BY DEBRAJ//

function attendanceregister_search_user_sessions($register, $userId, $fromdate , $todate) {
    global $DB;
    if($fromdate!='')
    {
        $filter1 = " AND login >= '".$fromdate."'";
    }
    else
    {
        $filter1 = '';
    ...
    if($todate!='')
    {
        $filter2 = " AND logout <= '".$todate."'";
    }
    else
    {
       $filter2 = ''; 
    }
   $userSessions = $DB->get_records_sql("SELECT * FROM {attendanceregister_session} WHERE register = '".$register->id."' AND userid = '".$userId."' ".$filter1.$filter2." ORDER BY login DESC");
 //echo  "SELECT * FROM {attendanceregister_aggregate} WHERE register = '".$register->id."' AND userid = '".$userId."' AND login > '".$fromdate."' AND logout < '".$todate."' ORDER BY login DESC"; die;
    return $userSessions;
}
 ...
 */
function attendanceregister_makeUrl($register, $id = null, $userId = null, $groupId = null, $action = null, $additionalParams = null, $forLog = false) {
