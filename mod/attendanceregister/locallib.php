...
function attendanceregister__get_all_users_aggregate_summaries_search($register,$from_date,$to_date) {
    global $DB;
    $params = array('register' => $register->id );
    $select = "register = :register AND (total = 1 OR grandtotal = 1)";
    return $DB->get_records_select('attendanceregister_aggregate', $select, $params);
  //  echo $sql_all = "SELECT * FROM {attendanceregister_aggregate} as x JOIN {attendanceregister_session} as z ON x.userid = z.userid "
   //     . " AND x.register = z.register WHERE z.login >= '".$from_date."' AND z.logout <= '".$to_date."' AND (x.total = 1 OR x.grandtotal = 1) "; die;
 //echo $sql_all = "SELECT * FROM {attendanceregister_aggregate} as a WHERE a.`register` = '".$register->id."' AND (a.total = 1 OR a.grandtotal = 1) AND"
         //. " a.`userid` IN (SELECT b.`userid` FROM {attendanceregister_session} as b WHERE b.`register` = '".$register->id."' AND b.`login` >= '".$from_date."' AND b.`logout` <= '".$to_date."')"; die;
 //echo $sql_all = "SELECT b.`userid` FROM {attendanceregister_session} as b WHERE b.`register` = '".$register->id."' AND b.`login` >= '".$from_date."' AND b.`logout` <= '".$to_date."'"; die;
