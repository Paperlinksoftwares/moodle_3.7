...
if(isset($_GET['searchbydate']) && $_GET['searchbydate']==1 && ($_GET['from_date']!='' || $_GET['to_date']!=''))
        {
            $this->userSessions = attendanceregister_search_user_sessions($register, $userId, strtotime($_GET['from_date']),strtotime($_GET['to_date']));
        }
        else
        {
    ...
    public function html_table() {
        ...
        $table->head = array(
            ...
            get_string('edit_session_login', 'attendanceregister'),
            get_string('edit_session_logout', 'attendanceregister'),
        ...
        if ( $this->userSessions ) {
            ...
            foreach ($this->userSessions as $session) {
               ...
               // echo $session->id; echo '<hr>';
                ...
                if($if_student==FALSE) {
                $editloginsession =  '<input type="text" class="edit_session_login_datetime" name="editloginsession'.$session->id.'" id="editloginsession'.$session->id.'" value="" style="width: 146px;" />';
                $editlogoutsession =  '<input type="text" class="edit_session_logout_datetime" name="editlogoutsession'.$session->id.'" id="editlogoutsession'.$session->id.'" value="" style="width: 146px;" />';
                $updatebutton =  '<input type="button" class="update-session" value=" Update " onclick=\'javascript: document.getElementById("session_id").value='.$session->id.'; document.getElementById("login_date").value=document.getElementById("editloginsession'.$session->id.'").value; document.getElementById("logout_date").value=document.getElementById("editlogoutsession'.$session->id.'").value; document.getElementById("login_date_old").value = '.$session->login.'; document.getElementById("logout_date_old").value = '.$session->logout.'; document.getElementById("f2").submit();\'>';
                $tableCelladd1 = new html_table_cell( $editloginsession );
                $tableCelladd2 = new html_table_cell( $editlogoutsession );
                $tableCelladd3 = new html_table_cell( $updatebutton );
                $tableRow->cells[] = $tableCelladd1;
                $tableRow->cells[] = $tableCelladd2;
                $tableRow->cells[] = $tableCelladd3;
                }
                $table->data[] = $tableRow;
            }
        } else {
