...
class Appointments extends CI_Controller {
    ...
    public function index($appointment_hash = '')
    {
	   $userid = $this->input->get('userid', TRUE);
		...
		$this->load->model('unitrequest_model');
        ...
        try
        {
			...
			$unit_requested_by_student = $this->unitrequest_model->get_info($userid);
            ...
            $view = [
				...
				'unit_requested_by_student'=>$unit_requested_by_student
    ...
    public function book_success($appointment_id)
    {
        ...
        /* QR CODE */
        $customer = $this->customers_model->get_row($appointment['id_users_customer']);
            if($customer['mobile_number']!='') { $mobile_num = $customer['mobile_number']; } else { $mobile_num = "NA"; }
            if($customer['city']!='') { $city = $customer['city']; } else { $city = "NA"; }
            if($customer['state']!='') { $state = $customer['state']; } else { $state = "NA"; }
            if($customer['zipcode']!='') { $zipcode = $customer['zipcode']; } else { $zipcode = "NA"; }
        	$this->load->library('ciqrcode');
			$qr_image=rand().'.png';
		//	$params['data'] = $this->input->post('qr_text');
			$params['data'] = "Customer Name : ".$customer['first_name']." ".$customer['last_name']
